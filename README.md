...
| `Enter` | open session |
| `j` / `k` | move selection |
| `1`–`9`, `0`, `11+` | focus by number (sidebar focused) |
| `⌘1`–`⌘9`, `⌘0` | focus by number (`⌘0` = 10). Works in Ghostty and **Cursor’s terminal pane** (install merges Cursor keybinds + silent `sessions focus` tasks) |
| `[` / `]` | narrower / wider sidebar (4 columns; sidebar focused) |
| `{` / `}` | narrower / wider sidebar (10 columns; sidebar focused) |
| `⌘⌥[` / `⌘⌥]` | resize sidebar from anywhere in **Cursor** (install merges keybinds + `sessions resize-sidebar` tasks). Prefer these over edge-drag in the IDE terminal. |

Install also tunes **VS Code / Cursor** terminal settings: `terminal.integrated.minimumContrastRatio` → `1` so truecolor is not washed out, and `terminal.integrated.rightClickBehavior` → `nothing` so the IDE context menu does not steal right-clicks from the sessions sidebar. A hybrid script (`sessions-terminal.js`, via Custom CSS and JS Loader) restores the IDE menu (New Terminal, Split, Copy, Paste, Clear, Kill, …) when you right-click the **workspace** pane; the **sidebar** still gets sessions menus. Without the script, **Shift+right-click** always shows the IDE menu. On macOS, setup disables **Press and Hold** for Cursor/VS Code so hold-`d` close mode is not interrupted by character alternatives. Re-run `./install.sh` or `bin/setup-vscode.sh` / `bin/setup-cursor.sh`, then reload the window (and “Enable Custom CSS and JS” once if needed).
| `⌘⇧[` / `⌘⇧]` | resize sidebar from **Ghostty** (Meta → tmux → `sessions resize-sidebar`) |
| `b` | toggle collapsed rail |
| top-right `[collapse] ◂` | collapse sidebar to status rail (click; same as `b`) |
