...
pub fn config_dir(home: &Path) -> PathBuf {
    if let Ok(dir) = std::env::var("SESSIONS_CONFIG_DIR") {
        let dir = dir.trim();
        if !dir.is_empty() {
            return PathBuf::from(dir);
        }
    }
    // Prefer XDG only when `home` is the process HOME. Tests pass a TempDir as
    // `home` and must not leak into ambient XDG_CONFIG_HOME (parallel tests race).
    let real_home = std::env::var_os("HOME").map(PathBuf::from);
    if real_home.as_ref().is_some_and(|h| h == home) {
...
/// Obot control-plane settings (`~/.config/sessions/obot.toml`).
pub fn obot_config_path(home: &Path) -> PathBuf {
    config_dir(home).join("obot.toml")
}

/// Per-agent MCP enablement matrix (`~/.config/sessions/mcp-enablement.toml`).
pub fn mcp_enablement_path(home: &Path) -> PathBuf {
    config_dir(home).join("mcp-enablement.toml")
}
