...
use super::{
    layout::{
        settings_section_rows, BOTTOM_CHROME_ROWS, LEAVE_BUTTON_ROWS, SETTINGS_BUTTON_ROWS,
        TOOLBAR_BUTTON_ROWS, TOOLBAR_SECTION_PAD, TOOLBAR_SECTION_ROWS, UPDATE_BOX_ROWS,
    },
    widgets::{
        ...
        ROW_LABEL_OFFSET, ROW_PRE_TRAILING_GAP, TRAILING_SLOT_WIDTH,
    },
    LayoutMetrics,
};
...
pub(crate) fn render_toolbar(
    ...
    let items: Vec<ListItem> = buttons
        ...
        .map(|button| {
            ...
            let active = button.action == ToolbarAction::NewSession && workspace_new_session_open;
...
pub(crate) fn render_update_box(
    ...
    if rows > 1 {
        if upgrade_hover {
            render_full_width_row_backdrop(frame, pane_area, box_rect.y.saturating_add(1), BG_HIGHLIGHT);
        }
    ...
    if rows > 2 {
        if dismiss_hover {
            render_full_width_row_backdrop(frame, pane_area, box_rect.y.saturating_add(2), BG_HIGHLIGHT);
        }
        ...
        frame.render_widget(
            List::new(vec![update_action_row("Remind me later", " ", width, dismiss_hover)])
                .style(Style::default().bg(BG_PANEL)),
            dismiss_rect,
        );
    }
...
pub fn update_banner_hover_from_mouse(
    y: u16,
    metrics: &LayoutMetrics,
) -> (bool, bool) {
