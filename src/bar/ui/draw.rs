...
use super::chrome::{
    render_bottom_chrome, render_collapse_control, render_toolbar, render_update_box,
};
use super::layout::UPDATE_BOX_ROWS;
use super::layout::{
    layout_plan_with_notepad, layout_regions, notepad_list_state, sidebar_trail_base_row,
    sidebar_trail_layout, total_list_rows, trail_row_from, NotepadListState, NotepadTrailRow,
    SESSION_BLOCK_PAD_LEFT, SESSION_BLOCK_PAD_RIGHT,
};
use super::notepad::{
    note_close_target_row, note_section_highlight, note_sections, notepad_body_line_spans,
    notepad_content_width, notepad_note_body_visible_rect, notepad_note_title_backdrop_bg_in_list,
    notepad_note_title_line, notepad_note_title_prefix, notepad_note_title_prefix_drag,
    notepad_note_title_row_style_drag, notepad_note_title_row_style_in_list,
    notepad_scrollbar_geometry, notepad_section_header_line, notepad_terminal_cursor_position,
    render_notepad_body_padding_backdrop, NoteDragState, NoteSection,
};
use super::sessions::{
    apply_group_highlight, close_target_row, compact_session_label, group_drag_row_backdrop,
    group_header_line, group_section_highlight, group_sections, group_toggle_row_is_selected,
    rail_status_glyph, render_sessions_title_hover_overlay, scroll_session_count,
    session_display_label, session_row_backdrop_bg, session_row_base_style,
    session_row_is_selected, session_trailing_badge, sessions_block_title, GroupDragState,
    GroupHighlight, GroupSection, RailStatusItem, RowKind,
};
use super::snapshot::{ChromeView, NotepadView, OverlayView, SessionsView, SidebarSnapshot};
use super::theme::*;
use super::widgets::{
    chrome_row_prefix, context_menu_items, context_menu_label, context_menu_rect,
    delete_note_confirm_rect, empty_trailing_slot, format_spinner_slot, full_width_line,
    full_width_spans, rename_targets_note, rename_targets_session, rename_terminal_cursor_position,
    render_delete_note_confirm, render_full_width_row_backdrop, render_notepad_context_menu,
    render_notepad_scrollbar, row_label_width, row_prefix, row_with_trailing_slot,
    run_spinner_glyph, spinner_badge_style, truncate, ContextMenuTarget, RenameState,
...
fn notepad_trail_list_item(
    ...
    checkbox_literal_edit: Option<usize>,
    _text_cursor: usize,
    ...
    match trail_row {
        ...
        NotepadTrailRow::NoteTitle { note_index } => {
            ...
            ListItem::new(notepad_note_title_line(
                prefix, title, line_width, row_style,
            ))
        }
        ...
        NotepadTrailRow::NoteBodySlot { note_index, slot } => {
            ...
            let body_selection = if is_active_body { selection } else { None };
            let literal_edit = if is_active_body {
                checkbox_literal_edit
            } else {
                None
            };
...
pub(crate) fn list_row_backdrop_bg(
    ...
    if row_idx < trail_base {
        ...
        return match row {
            ...
            RowKind::Group { .. } => group_drag_row_backdrop(sections, rows, row_idx, group_drag)
                .or_else(|| {
                    (!group_drag.active() && group_hover_row == Some(row_idx))
                        .then_some(BG_HIGHLIGHT)
                }),
    ...
    let trail_idx = row_idx.saturating_sub(trail_base);
    // Backdrop for note rows.
    let trail_row = note_state.trail_layout().get(trail_idx).cloned()?;
    let note_highlight = note_section_highlight(note_drag_sections, trail_idx, note_drag);
    match trail_row {
...
/// Pad a 1-cell glyph into `width` columns with **optical** centering.
///
/// Naive `floor((w-1)/2)` left-pad on a 4-col rail yields `" X  "` (1 left / 2 right),
/// which reads stuck to the window edge. Prefer slightly more left pad (`"  X "`) so
/// chips sit in the visual middle of the strip (tmux's right pane border also pulls
/// the eye rightward).
pub(crate) fn rail_centered_cell(ch: char, width: u16) -> String {
    let w = width.max(1) as usize;
    if w == 1 {
        return ch.to_string();
    }
    // ceil((w-1)/2) via integer: w/2 → width 4 ⇒ left 2, right 1.
    let left = w / 2;
    let right = w.saturating_sub(1 + left);
    format!("{}{}{}", " ".repeat(left), ch, " ".repeat(right))
}

/// Micro status rail when the sidebar is collapsed (auto, `b`, or divider snap).
///
/// - **Top:** expand control (`▸`) — open the full sidebar (muted chrome, no
///   selected/panel backdrop).
/// - **Body:** status **icons only** on the **same Y as expanded session rows** —
///   colored glyphs, no section selection / hover / status fills.
///
/// Click anywhere / drag open / press `b` to expand.
pub fn draw_collapsed_rail(
    frame: &mut Frame,
    ...
    use super::layout::{RAIL_EXPAND_HEADER_ROWS, RAIL_EXPAND_ICON};
    use super::sessions::rail_item_screen_y;
    ...
    // Expand affordance only — no BG_PANEL / selection-style highlight on the rail.
    let expand_style = Style::default().fg(TEXT_SECONDARY).bg(BG_BASE);
    let header_rows = RAIL_EXPAND_HEADER_ROWS.min(area.height);
    let header_bottom = area.y.saturating_add(header_rows);

    // Expand control at the top — full-width hit target, same optical center as chips.
    if header_rows >= 1 {
        frame.render_widget(
            Paragraph::new(rail_centered_cell(RAIL_EXPAND_ICON, area.width)).style(expand_style),
            Rect::new(area.x, area.y, area.width, 1),
        );
        // One blank gap under the control so it reads as chrome, not a status chip.
        if header_rows >= 2 {
            frame.render_widget(
                Paragraph::new(" ".repeat(area.width as usize)).style(Style::default().bg(BG_BASE)),
                Rect::new(area.x, area.y.saturating_add(1), area.width, 1),
            );
        }
    }

    // Status icons aligned to expanded list rows (scrolled the same way).
    // Skip any row that would paint over the expand header. No bold/selection
    // emphasis for the active session — the rail is status-only.
    for item in items {
        let Some(y) = rail_item_screen_y(item.list_row, scroll, list_top_y, list_height) else {
            continue;
        };
        if y < header_bottom || y >= area.y.saturating_add(area.height) {
            continue;
        }
        let (ch, style) = rail_status_glyph(item.kind, anim_frame);
        // Full-width cell with optical centering (extra left pad on even widths).
        frame.render_widget(
            Paragraph::new(rail_centered_cell(ch, area.width)).style(style),
...
pub fn draw(frame: &mut Frame, snap: &SidebarSnapshot<'_>) {
    ...
    let NotepadView {
        ...
        checkbox_literal_edit: notepad_checkbox_literal_edit,
        suppress_terminal_cursor: notepad_suppress_terminal_cursor,
    ...
    // Top-right chrome: `[collapse] ◂` on the pad row above toolbar buttons.
    render_collapse_control(frame, &plan.metrics, collapse_control_hover);
    ...
    // Build the trailing layout once per frame — indexing is O(1).
    let trail = sidebar_trail_layout(&notepad_state);
    let total_rows = total_list_rows(rows.len(), sessions_expanded, &notepad_state);
    ...
    let visible: Vec<ListItem> = (scroll..scroll.saturating_add(body_height))
        ...
        .map(|row_idx| {
            if row_idx >= trail_base {
                let trail_idx = row_idx.saturating_sub(trail_base);
                let trail_row =
                    trail_row_from(&trail, trail_idx).unwrap_or(NotepadTrailRow::SectionPad);
            ...
            match row {
                RowKind::Empty(label) => {
                    ...
                    let is_boot = label == "Starting…" || label == "Reconnecting…";
                    let (trailing, trailing_style) = if is_boot {
                        (
                ...
                RowKind::Group { label, collapsed } => {
                    ...
                    ListItem::new(group_header_line(
                        label,
                        *collapsed,
                        group_highlight,
                        hovered,
                ...
                RowKind::Session { session } => {
                    ...
                    let title_spans = if let Some(rename) =
    ...
    if notepad_focused && notepad_expanded {
        if let Some(note_index) = active_note_index {
            if let Some(pos) = notepad_terminal_cursor_position(
                ...
                notepad_selection,
                notepad_checkbox_literal_edit,
                notepad_suppress_terminal_cursor,
            ) {
    ...
    // IDE hosts: right-edge grip so resize is discoverable without OSC 22.
    if crate::bar::host_terminal::detect().uses_edge_resize() && area.width > 0 {
        let grip_style = Style::default().fg(PATH_FG).bg(BG_PANEL);
        let x = area.x.saturating_add(area.width.saturating_sub(1));
        for row in 0..area.height {
            frame.render_widget(
                Paragraph::new(Line::from(Span::styled("│", grip_style))),
                Rect {
                    x,
    ...
    if let Some(menu) = context_menu {
        if matches!(menu.target, ContextMenuTarget::Notepad { .. }) {
            ...
            for (idx, action) in context_menu_items(&menu.target).iter().enumerate() {
                let hovered = menu.hover == Some(*action);
                let row_bg = if hovered { BG_SELECTED } else { BG_PANEL };
                let style = Style::default().fg(TEXT_SELECTED).bg(row_bg);
                let item_rect = Rect {
                    x: rect.x,
                    y: rect.y + idx as u16,
                    width: rect.width,
