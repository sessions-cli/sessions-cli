...
pub struct ContextMenu {
    ...
    pub x: u16,
    pub y: u16,
    /// Enabled action under the pointer, if any.
    pub hover: Option<ContextMenuAction>,
}
...
/// Max chars for session/note renames (titles stay short).
const RENAME_MAX_CHARS_TITLE: usize = 80;

fn rename_max_chars(_rename: &RenameState) -> usize {
    RENAME_MAX_CHARS_TITLE
}
...
pub fn rename_apply_char(rename: &mut RenameState, c: char) {
    ...
    if rename.buffer.chars().count() < rename_max_chars(rename) {
...
pub fn rename_apply_paste(rename: &mut RenameState, text: &str) {
    ...
    let max = rename_max_chars(rename);
    for ch in text.chars() {
        if rename.buffer.chars().count() >= max {
            break;
        }
...
/// One-letter quick-launch badges on pwd group headers (same width as `[+]`).
pub(crate) const GROUP_GROK_ICON: &str = "[G]";
pub(crate) const GROUP_CODEX_ICON: &str = "[X]";
pub(crate) const GROUP_CLAUDE_ICON: &str = "[C]";
pub(crate) const GROUP_OPENCODE_ICON: &str = "[O]";
...
pub(crate) fn render_notepad_scrollbar(
    frame: &mut Frame,
...
pub fn context_menu_items(target: &ContextMenuTarget) -> &'static [ContextMenuAction] {
    match target {
        ...
        ContextMenuTarget::Note { .. } => &[ContextMenuAction::Rename, ContextMenuAction::Delete],
...
fn context_menu_row_bg(hovered: bool) -> Color {
    if hovered {
        BG_SELECTED
    } else {
        BG_PANEL
...
fn context_menu_notepad_line(
    ...
    let row_bg = context_menu_row_bg(hovered && enabled);
...
pub(crate) fn render_notepad_context_menu(frame: &mut Frame, menu: &ContextMenu, area: Rect) {
    ...
    for (idx, action) in context_menu_items(&menu.target).iter().enumerate() {
        ...
        frame.render_widget(
            Paragraph::new(context_menu_notepad_line(*action, enabled, hovered))
