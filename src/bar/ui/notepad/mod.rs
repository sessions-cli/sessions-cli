...
use super::theme::*;
use super::widgets::{
    chrome_row_prefix, empty_trailing_slot, format_trailing_slot, full_width_spans,
    group_add_badge_style, notepad_scrollbar_track_x, rename_targets_note, truncate,
    NotepadScrollbar, RenameState, CHROME_ROW_PREFIX, GROUP_ADD_ICON, ROW_PRE_TRAILING_GAP,
    TRAILING_SLOT_WIDTH,
};
use super::{
    is_group_add_click, notepad_note_body_row_range, notepad_trail_layout, notepad_trail_row_at,
    pointer_in_list_body, terminal_list_area, LayoutMetrics, NotepadListState, NotepadTrailRow,
};
...
pub struct NoteDragState {
    ...
    /// True once the pointer leaves the source note — expand stays click-only until then.
    pub dragged: bool,
...
pub(crate) fn note_section_highlight(
    ...
    // Match PWD groups: show the drag grip only after leaving the source note.
    if !note_drag.active() || !note_drag.dragged {
        return None;
    }
...
pub fn notepad_note_body_visible_rect(
    ...
    let (body_start, body_end) = notepad_note_body_row_range(note_index, visible_sessions, state)?;
...
pub fn notepad_note_title_line(
    ...
    full_width_spans(
        vec![
            ...
            Span::styled(" ".repeat(label_width.saturating_sub(title_len)), row_style),
        ],
        line_width,
        row_style,
    )
...
pub fn notepad_body_line_spans(
    ...
    body_line_spans(
        ...
        select_bg,
        selection,
        literal_edit_bracket,
        NOTEPAD_BODY_INDENT,
    )
}

fn md_checkbox_span_selected(
    line_start: usize,
    checkbox: crate::bar::notepad::MdCheckboxSpan,
    selection: Option<(usize, usize)>,
) -> bool {
    let abs_start = line_start + checkbox.bracket_start;
    let abs_end = line_start + checkbox.bracket_end;
    selection.is_some_and(|(start, end)| start < end && start <= abs_start && abs_end <= end)
        || (abs_start..abs_end).any(|abs| notepad_char_selected(abs, selection))
}
...
pub(crate) fn body_line_spans(
    ...
    let checkbox_unchecked = Style::default().bg(body_bg);
    ...
    let task = crate::bar::notepad::md_task_list_line(line);
    let checkbox = task.map(|task| crate::bar::notepad::MdCheckboxSpan {
        bracket_start: task.bracket_start,
        bracket_end: task.bracket_end,
        checked: task.checked,
    });
    let show_glyph = crate::bar::notepad::md_checkbox_show_glyph(
        line,
        line_start,
        selection,
        literal_edit_bracket,
    ...
    while col < line_len {
        if show_glyph {
            if let Some(task) = &task {
                if col >= task.indent && col < task.bracket_start {
                    col = task.bracket_start;
                    continue;
                }
            }
        }
        if let Some(cb) = checkbox.filter(|cb| show_glyph && col == cb.bracket_start) {
            flush(&mut run, run_style, &mut spans);
            let glyph = if cb.checked {
                crate::bar::notepad::format_md_checkbox_checked_glyph()
            } else {
                crate::bar::notepad::MD_CHECKBOX_UNCHECKED_GLYPH.to_string()
            };
            let style = if md_checkbox_span_selected(line_start, cb, selection) {
                selected
            } else if cb.checked {
                Style::default()
                    .fg(DONE_FG)
...
pub fn notepad_terminal_cursor_position(
    ...
    selection: Option<(usize, usize)>,
    literal_edit_bracket: Option<usize>,
    suppress_terminal_cursor: bool,
) -> Option<Position> {
    if !state.section_expanded || !notepad_focused || suppress_terminal_cursor {
        return None;
    }
    if crate::bar::notepad::md_checkbox_terminal_cursor_hidden(
        text,
        cursor,
        selection,
        literal_edit_bracket,
    ...
    if list_row_idx >= body_end
        || list_row_idx < scroll
        || list_row_idx >= scroll.saturating_add(body_height)
    {
        return None;
    }
    let col_in_line = cursor.saturating_sub(display_line.start);
    let display_col = crate::bar::notepad::display_col_for_raw_col_in_line(
        &display_line.text,
        col_in_line,
        display_line.start,
        selection,
    ...
    Some(Position::new(
        terminal_area
            .x
            .saturating_add(indent_len.saturating_add(display_col as u16)),
...
pub fn notepad_cursor_from_mouse(
    ...
    let raw_col = crate::bar::notepad::raw_col_for_display_col_in_line(
        &display_line.text,
        text_col,
        display_line.start,
        None,
    ...
    .min(display_line.text.chars().count());
    Some(display_line.start + raw_col)
}
...
pub fn notepad_selection_cursor_from_mouse(
    ...
    if row_idx < body_start {
        ...
        .min(first.text.chars().count());
        return Some(first.start + raw_col);
    }
    if row_idx >= body_end {
        ...
        .min(last.text.chars().count());
        return Some(last.start + raw_col);
    }
    None
}
...
pub fn notepad_hit_from_mouse(
    ...
    let trail_row = notepad_trail_row_at(trail_idx, state)?;
    match trail_row {
        ...
        NotepadTrailRow::NoteTitle { note_index } => Some(NotepadHit::NoteTitle { note_index }),
        NotepadTrailRow::NotesToggle { .. } => Some(NotepadHit::NotesToggle),
        NotepadTrailRow::NoteBodyPad { note_index }
        | NotepadTrailRow::NoteBodySlot { note_index, .. } => {
...
pub fn notepad_checkbox_bracket_from_mouse(
    column: u16,
    y: u16,
    metrics: &LayoutMetrics,
    scroll: usize,
    ...
    let wrapped = crate::bar::notepad::wrapped_display_lines(text, content_width);
    let display_line = wrapped.get(line_idx)?;
    if !crate::bar::notepad::md_checkbox_display_hit(
        &display_line.text,
        text_col,
        display_line.start,
        selection,
    ...
    let cb = crate::bar::notepad::md_checkbox_in_line(&display_line.text)?;
    Some(display_line.start + cb.bracket_start)
}

pub fn notepad_checkbox_hover_from_mouse(
    column: u16,
    y: u16,
    metrics: &LayoutMetrics,
    scroll: usize,
    ...
    let Some(line_idx) = notepad_line_from_mouse(
        y,
        metrics,
        scroll,
        visible_sessions,
    ...
    let content_width = notepad_content_width(metrics.list_line_width, true);
    let wrapped = crate::bar::notepad::wrapped_display_lines(text, content_width);
    let Some(display_line) = wrapped.get(line_idx) else {
        return false;
    };
    crate::bar::notepad::md_checkbox_display_hit(
        &display_line.text,
        text_col,
        display_line.start,
        selection,
...
pub fn notepad_note_title_row_style(editing: bool, is_active: bool, is_hovered: bool) -> Style {
...
pub fn row_is_note_title(
    ...
    let trail_idx = row_idx.saturating_sub(trail_base);
    matches!(
        notepad_trail_row_at(trail_idx, note_state),
