...
fn session_row_is_selected_follows_preserved_id_during_drag() {
    ...
    assert!(session_row_is_selected(&session, 3, 3, &GroupDragState::default()));
...
fn notepad_trail_layout_caps_collapsed_note_titles_with_toggle() {
    ...
    assert!(!layout.iter().any(|row| matches!(row, NotepadTrailRow::NotesToggle { .. })));
    assert_eq!(
        layout
            .iter()
...
fn ensure_active_note_visible_does_not_jump_to_list_bottom() {
    ...
    let (_, body_end) =
        notepad_note_body_row_range(1, trail_base, &state).expect("note body");
...
fn notepad_cursor_from_mouse_maps_click_to_text_position() {
    ...
    assert_eq!(
        notepad_cursor_from_mouse(5, body_y, &metrics, 0, 0, &state, 0, text, 0),
        Some(2)
    );
    let second_line_y = body_y + 1;
    assert_eq!(
        notepad_cursor_from_mouse(5, second_line_y, &metrics, 0, 0, &state, 0, text, 0),
        Some(8)
    );
}
...
fn session_label_uses_console_for_idle_shell() {
    ...
    session.title = "superflip · console".into();
    session.project = "superflip".into();
    assert_eq!(session_display_label(&session), "console");
}

#[test]
fn session_label_normalizes_legacy_raw_terminal_to_console() {
    let mut session = sample_session("superflip", 1, 0);
    session.title = "superflip · raw terminal".into();
    session.project = "superflip".into();
    assert_eq!(session_display_label(&session), "console");
}
...
#[test]
fn toolbar_coming_soon_actions_are_search_automations_mcps_and_skills() {
...
fn notepad_note_title_prefix_aligns_with_notes_header_label() {
    ...
    assert_eq!(notepad_note_title_prefix(true, false).chars().count(), n_in_header);
    assert_eq!(notepad_note_title_prefix(false, true).chars().count(), n_in_header);
    assert_eq!(
...
fn notepad_note_title_drag_prefix_preserves_title_offset() {
    ...
    assert_eq!(
        notepad_note_title_prefix_drag(false, false, Some(GroupHighlight::Source)).chars().count(),
        NOTEPAD_NOTE_TITLE_OFFSET
    );
    assert_eq!(
        notepad_note_title_prefix_drag(false, false, Some(GroupHighlight::Target)).chars().count(),
        NOTEPAD_NOTE_TITLE_OFFSET
    );
    assert_eq!(
        notepad_note_title_prefix_drag(false, false, None).chars().count(),
        NOTEPAD_NOTE_TITLE_OFFSET
    );
}
...
fn notepad_scrollbar_geometry_sizes_thumb_to_viewport_ratio() {
    ...
    let scrollbar = notepad_scrollbar_geometry(
        terminal, 0, 18, 3, &state, 0, &text, 0, 40,
    )
...
fn notepad_scroll_from_thumb_drag_tracks_pointer_with_grab_offset() {
    ...
    assert_eq!(
        notepad_scroll_from_thumb_drag(5, &scrollbar, 8, 0),
        0
    );
    assert_eq!(
        notepad_scroll_from_thumb_drag(14, &scrollbar, 8, 0),
        8
    );
    assert_eq!(
        notepad_scroll_from_thumb_drag(13, &scrollbar, 8, 1),
        7
    );
}
...
fn group_add_click_detects_trailing_icon_on_group_row() {
    ...
    assert_eq!(
        group_add_click(inside_slot, row_y, &metrics, 0, rows.len(), &rows),
        Some("~/a")
    );
    assert_eq!(
        group_add_click(trailing_col, row_y, &metrics, 0, rows.len(), &rows),
        Some("~/a")
    );
    assert!(group_add_click(
