...
impl App {
    ...
    pub(crate) fn handle_terminal_resize(&mut self, width: u16, height: u16) {
        // Live in-bar edge drag already owns width; Resize is just the echo of our
        // own resize-pane. Avoid re-sync + force_redraw (that made IDE drag jumpy).
        if self.edge_resize_active {
            let stable_h = self
                .sidebar_layout_height_freeze
                .or(self.render_cache.size.map(|(_, h)| h))
                .unwrap_or(height);
            self.render_cache.size = Some((width, stable_h));
            self.user_pane_width = Some(width);
            return;
        }
        let last_height = self.render_cache.size.map(|(_, h)| h);
        let height_changed = last_height.is_none_or(|h| h != height);
        // During host-width resize, terminal height often flickers by a row — that
        // would recompute list_top_y and make the list jump. Only accept a real
        // height change when not mid host-resize (or change is large).
        let accept_height_change = height_changed
            && (!self.in_sidebar_host_resize_grace()
                || last_height.is_none_or(|h| h.abs_diff(height) > 1));
        if accept_height_change {
        ...
        // Pane Resize fires for both host-terminal resizes and divider drags.
        // Force sync so host-edge drags re-pin preferred every event (the 250ms poll
        // alone leaves tmux free to squash the sidebar between ticks → jumpiness).
        // Divider drags still adopt width when client width is stable (see sync).
    ...
    pub(crate) fn ensure_active_session_visible(&mut self) {
        ...
        self.ensure_session_id_visible(&active_id);
    }

    /// Expand a directory group when `session_id` would be clipped by the
    /// per-group cap (`MAX_THREADS_PER_GROUP`). Without this, rebuild falls back
    /// to the first selectable row whenever messaged_at reordering pushes the
    /// selected/active session out of the visible top-N — which feels like the
    /// highlight "jumping about".
    pub(crate) fn ensure_session_id_visible(&mut self, session_id: &str) {
        ...
        let cwd = session.cwd_label.clone();
        if self.expanded_groups.contains(&cwd) || self.folded_groups.contains(&cwd) {
            return;
        }
        let mut group: Vec<Session> = self
            .sessions
            .iter()
            .filter(|s| s.cwd_label == cwd)
            .cloned()
            .collect();
        group.sort_by(|a, b| a.cmp_within_group(b));
        let visible = group_order::visible_sessions_in_group(&group, false);
        if visible.iter().any(|s| s.id == session_id) {
            return;
        }
    ...
    /// True when the list highlight is on the currently active session (user is
    /// "tracking" focus). Browse-with-arrows onto another row must not be stolen
    /// by a later is_active flip.
    pub(crate) fn selection_tracks_active(&self) -> bool {
        let Some(selected) = self.session_at(self.selected) else {
            // Group toggle / empty — treat as free to follow active.
            return true;
        };
        selected.is_active
    }

    /// Restore selection after a row rebuild without always pinning scroll.
    /// Index changes from within-group messaged_at reordering should not yank
    /// the viewport unless the row left the visible window (handled on draw).
    fn restore_selected_row(&mut self, idx: usize) {
        if self.selected != idx {
            self.selected = idx;
            // Keep ssn in sync without scheduling a save storm on pure reorder.
            ...
            // Only scroll-sync when the restored row is outside the last known
            // viewport — avoids list jumpiness on every sort reorder.
            if let Some(body_h) = self.render_cache.size.map(|(_, h)| h as usize) {
                let list_h = body_h.max(1);
                if self.selected < self.scroll
                    || self.selected >= self.scroll.saturating_add(list_h)
                {
    ...
    /// Ensure every live PWD is present in `group_order` before reorder/preview.
    /// Saved order can be partial (new dirs, or test pollution) — without this,
    /// `reorder` silently no-ops because the labels are not in the vec.
    pub(crate) fn sync_group_order_labels(&mut self) {
    ...
    /// Materialize live PWDs into saved group order (and drop junk/stale labels).
    /// Returns true when the in-memory order changed (caller should rebuild rows).
    pub(crate) fn persist_group_order_from_sessions(&mut self) -> bool {
        if self.sessions.is_empty() {
            return false;
        }
        group_order::reconcile_and_save(&self.config, &mut self.group_order, &self.sessions)
            .unwrap_or_default()
    }

    pub(crate) fn group_order_with_live_labels(&self) -> Vec<String> {
        let mut order = self.group_order.clone();
        group_order::ensure_labels(&mut order, &group_order::unique_labels(&self.sessions));
        order
    }

    pub(crate) fn effective_group_order(&self) -> Vec<String> {
        let order = self.group_order_with_live_labels();
        match (&self.group_drag.source, &self.group_drag.hover) {
            (Some(from), Some(to)) if from != to => group_order::preview_order(&order, from, to),
            _ => order,
        }
    }
    /// Refresh session clones embedded in row items without rebuilding group structure.
    pub(crate) fn sync_row_sessions(&mut self) {
        use super::daemon::session_row_render_eq;
        ...
        for row in &mut self.rows {
            ...
            if let Some(fresh) = self.sessions.iter().find(|s| s.id == session.id) {
                if !session_row_render_eq(session, fresh) {
                    *session = fresh.clone();
                    changed = true;
                }
            }
    ...
    pub(crate) fn rebuild_rows(&mut self) {
        ...
        // Resolve which session to keep highlighted across a rebuild:
        // - Cold start: prefer persisted SSN, then current row.
        // - After init: if the row under `selected` still matches the last
        //   set_selected SSN, trust the live row. If they disagree (index went
        //   stale after a group reorder), trust the durable SSN instead of
        //   silently adopting whatever session now sits at that index.
        let previously_selected_session =
            self.group_drag.preserved_session_id.clone().or_else(|| {
                let live_id = self
                ...
                let ssn_id = self
                    ...
                    .and_then(|ssn| {
                        self.sessions
                            .iter()
                            .find(|session| session.sessions_session_id.as_deref() == Some(ssn))
                            .map(|session| session.id.clone())
                    });
                if !self.selection_initialized {
                    return ssn_id.or(live_id);
                }
                match (live_id, ssn_id) {
                    (Some(live), Some(ssn)) if live != ssn => Some(ssn),
                    (Some(live), _) => Some(live),
                    (None, Some(ssn)) => Some(ssn),
                    (None, None) => None,
                }
            });
        ...
        // Keep both active *and* the row the user is highlighting out of the
        // collapsed top-N clip before we build rows.
        if let Some(ref id) = previously_selected_session {
            self.ensure_session_id_visible(id);
        }
        ...
        // Cold start / reconnect: replace the bare empty list with a calm status
        // line so the sidebar never looks broken while the daemon is catching up.
        if self.sessions.is_empty() {
            if let Some(label) = self.boot_empty_label() {
                self.rows = vec![RowKind::Empty(label.into())];
            }
        }
        ...
        // `pending_focus_tab_index` owns optimistic *is_active* only
        // (`reconcile_pending_focus` / `reapply_pending_focus_after_patch`). It must
        // not re-pin the list highlight — after click-activate, arrow-key browse
        // would snap back on every patch/rebuild while pending was still set.
                    ...
                    previously_selected_group.as_deref().and_then(|cwd_label| {
                        self.rows
                            ...
                            .find_map(|(idx, row)| match row {
                                RowKind::GroupToggle {
                                    cwd_label: label, ..
                                } if label == cwd_label => Some(idx),
                                _ => None,
                            })
                    })
                });
            if let Some(idx) = restored {
                self.restore_selected_row(idx);
            }
            ...
            if let Some(idx) = self.selectable.iter().copied().find(|&row_idx| {
                ...
                self.restore_selected_row(idx);
            } else if self.sessions.iter().any(|s| s.id == selected_id) {
                // Still live but not in the list (folded group, race). Keep the
                // current index if it is still selectable; never jump to first.
                if !self.selectable.contains(&self.selected) {
                    if let Some(&first) = self.selectable.first() {
                        self.set_selected(first);
                    }
                }
            } else if !self.selectable.contains(&self.selected) {
                // Session closed — only then fall back.
                if let Some(&first) = self.selectable.first() {
                    self.set_selected(first);
                }
            }
            ...
            if let Some(idx) = self
                ...
                .iter()
                .copied()
                .find(|&row_idx| ui::group_toggle_at(&self.rows, row_idx) == Some(cwd_label))
            {
                self.restore_selected_row(idx);
            } else if !self.selectable.contains(&self.selected) {
                if let Some(&first) = self.selectable.first() {
    ...
    pub(crate) fn focus_row(&mut self, row_idx: usize) -> bool {
        ...
        let focus_ok = if let Some(ref ssn) = sessions_session_id {
            ...
            crate::daemon::tmux::select_window(&self.config.tmux_session, focused_tab_index).is_ok()
        };
        if focus_ok {
    ...
    pub(crate) fn jump_to_ordinal(&mut self, ordinal: u32) {
        ...
        if let Some(&idx) = session_rows.get(ordinal.saturating_sub(1) as usize) {
            crate::telemetry::record_feature(
                crate::telemetry::FeatureId::ListFocusNumber,
                crate::telemetry::feature::Source::Keyboard,
            );
            self.set_selected(idx);
            self.force_redraw();
    ...
    pub(crate) fn list_hover_updates_suppressed(&self, mouse_row: u16) -> bool {
        self.suppress_list_hover_after_group_drag && self.suppress_list_hover_y == Some(mouse_row)
    }
...
mod tests {
    ...
    #[test]
    fn rebuild_expands_group_so_selected_session_stays_visible() {
        // Groups larger than MAX_THREADS_PER_GROUP hide older rows. When
        // messaged_at reordering pushes the selected session out of the top-N,
        // we must expand rather than jump the highlight to the first row.
        use crate::session::group_order::MAX_THREADS_PER_GROUP;
        let config = Config::default();
        let mut app = App::new(&config).unwrap();
        app.selection_initialized = true;
        let n = MAX_THREADS_PER_GROUP + 2;
        app.sessions = (1..=n as u32)
            .map(|i| {
                let mut s = sample_session_in_group(
                    &format!("tmux:win:{i}"),
                    i,
                    &format!("thread-{i}"),
                    "~/tmp/overflow",
                ...
                // Oldest first by tab, newest messaged_at for low tab indexes so
                // high tabs fall below the cap when collapsed.
                s.messaged_at = Some(Utc::now() - chrono::Duration::minutes(i as i64));
                s
            })
            .collect();
        app.rebuild_rows();
        // Select the oldest (last in sort) — would be clipped without expand.
        let oldest_id = format!("tmux:win:{n}");
        // Force selection via sessions list even if currently hidden.
        app.selected = 0;
        app.sidebar_ui_selected_sessions_session_id = None;
        // Manually point selection at oldest by temporarily expanding.
        app.expanded_groups.insert("~/tmp/overflow".into());
        app.rebuild_rows();
        let oldest_row = app
            .session_row_index(&oldest_id)
            .expect("oldest visible when expanded");
        app.set_selected(oldest_row);
        // Collapse and rebuild — must re-expand to keep selection.
        app.expanded_groups.remove("~/tmp/overflow");
        app.rebuild_rows();
        assert!(
            app.expanded_groups.contains("~/tmp/overflow"),
            "group must auto-expand to keep selected session visible"
        );
        assert_eq!(
            app.session_at(app.selected).map(|s| s.id.as_str()),
            Some(oldest_id.as_str()),
            "selection must stay on the same session, not jump to first"
        );
    }
    ...
    #[test]
    fn rebuild_rows_does_not_override_browse_with_pending_focus() {
        // Pending focus keeps optimistic is_active, but arrow-key browse onto
        // another row must survive rebuilds (patches while pending is open).
        let config = Config::default();
        let mut app = App::new(&config).unwrap();
        app.selection_initialized = true;
        app.sessions = vec![
        ...
        app.pending_focus_tab_index = Some(1);
        let second = app.session_row_index("tmux:win:2").unwrap();
        app.set_selected(second);
        app.rebuild_rows();
        assert_eq!(
            app.session_at(app.selected).map(|s| s.id.as_str()),
            Some("tmux:win:2"),
            "pending focus must not snap selection back after browse"
        );
        assert_eq!(app.pending_focus_tab_index, Some(1));
    }
