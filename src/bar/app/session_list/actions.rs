...
impl App {
    ...
    pub(crate) fn coalesce_mouse_moves(
        ...
        let mut latest: Option<MouseEvent> = None;
        ...
        if let Some(mouse) = latest {
            self.note_pointer_activity(&mouse, size.width);
            self.last_mouse = Some(mouse);
            if self.note_drag.pending() || self.group_drag.pending() {
    ...
    pub(crate) fn update_group_drag_hover(&mut self, mouse: &MouseEvent, metrics: &ui::LayoutMetrics) {
        let source = self.group_drag.source.as_deref();
        // Hit-test against the saved group order, not the live preview layout.
        // Preview reorder moves sections under the cursor and flips the target back
        // to the source — the grab/flash/jump loop users see while dragging.
        ...
        .and_then(|row_idx| {
            source.and_then(|from| ui::group_drag_target(&hit_rows, row_idx, from))
        })
    ...
    pub(crate) fn create_session(&mut self) {
        ...
        if let Ok(window_index) =
            crate::daemon::tmux::create_console_window(&self.config)
        {
