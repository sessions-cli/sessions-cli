...
impl App {
    ...
    pub(crate) fn update_note_drag_hover(&mut self, mouse: &MouseEvent, metrics: &ui::LayoutMetrics) {
    ...
    pub(crate) fn ensure_active_note(&mut self) {
        ...
        let has_active = self.active_note_id.as_ref().is_some_and(|id| {
    ...
    pub(crate) fn note_index_for_title_row(&self, row_idx: usize) -> Option<usize> {
        ...
        match ui::sidebar_trail_row_at(trail_idx, &self.notepad_list_state())? {
    ...
    pub(crate) fn copy_notepad_selection(&mut self) -> bool {
        let Some((start, end)) = self.notepad_editor.selection.filter(|(start, end)| start < end) else {
            return false;
        };
    ...
    pub(crate) fn open_notepad_context_menu(&mut self, mouse: &MouseEvent, metrics: &ui::LayoutMetrics) {
        self.rename = None;
        self.notepad_editor.drag_selecting = false;
        self.notepad_editor.select_anchor = None;
        ...
        self.context_menu = Some(ui::ContextMenu {
            target: ui::ContextMenuTarget::Notepad {
    ...
    pub(crate) fn handle_notepad_note_title_click(&mut self, mouse: &MouseEvent, note_index: usize) {
    ...
    pub(crate) fn handle_notepad_scrollbar_click(
        ...
        let Some(target) =
            editor::scroll_from_track_click(mouse.row, &scrollbar, max_scroll)
        else {
            return;
        };
    ...
    pub(crate) fn update_notepad_scrollbar_drag(
        ...
        let next = editor::scroll_from_thumb_drag(
            mouse.row,
            &scrollbar,
            max_scroll,
            grab_offset,
        ...
        if click_count == 2 {
            ...
            if let Some(cursor) = cursor {
                if let Some((start, end)) =
                    notepad::word_range_at(self.active_note_text(), cursor)
                {
                    self.notepad_editor.selection = Some((start, end));
                    self.focus_notepad_at_cursor(Some(end));
                    self.copy_notepad_selection();
    ...
    pub(crate) fn update_notepad_drag_selection(&mut self, mouse: &MouseEvent, metrics: &ui::LayoutMetrics) {
        let Some(note_index) = self.active_note_index() else {
            return;
        };
        ...
        let changed = self.notepad_editor.selection != Some((start, end)) || self.notepad_editor.cursor != head;
        if !changed {
            return;
        }
        self.notepad_editor.selection = Some((start, end));
...
mod tests {
    ...
    #[test]
        fn delete_note_confirm_requires_typing_yes() {
            ...
            app.handle_delete_note_confirm_key(key, &mut terminal).unwrap();
    ...
    #[test]
        fn note_title_hover_uses_text_cursor() {
            ...
            let title_row = ui::notepad_note_title_row_index(0, app.rows.len(), &app.notepad_list_state())
                .expect("title row");
            let row_y = metrics.list_top_y.saturating_add(title_row.saturating_sub(app.scroll) as u16);
