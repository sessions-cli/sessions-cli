...
pub fn settings_layout(inner: Rect, rows: &[SettingsRow]) -> SettingsLayout {
    ...
    // Fixed chrome around the list — always keep title/CTA/hint inside the pane.
    let chrome_h =
        TITLE_ROWS + HEADER_LIST_GAP + CTA_TOP_GAP + CTA_BUTTON_ROWS + HINT_TOP_GAP + HINT_ROWS;
    // Shrink the list when content is taller than the pane so draw never paints past the buffer
    // (adding PWD quick-launch rows made full-height settings panic on short terminals).
    let list_h = list_lines.min(inner.height.saturating_sub(chrome_h).max(1));
    let used = chrome_h.saturating_add(list_h);
    let content_y = if used <= inner.height {
        inner.y + inner.height.saturating_sub(used) / 2
    } else {
        inner.y
    };
    ...
    let header = Rect {
        ...
        y,
        width: w,
        height: TITLE_ROWS.min(bottom.saturating_sub(y)),
    };
    ...
    let header_list_gap = Rect {
        ...
        y,
        width: w,
        height: HEADER_LIST_GAP.min(bottom.saturating_sub(y)),
    };
    y = y.saturating_add(header_list_gap.height);
    let list = Rect {
        x,
        y,
        width: w,
        height: list_h.min(bottom.saturating_sub(y)),
    };
    y = y.saturating_add(list.height);
    let cta_top_gap = Rect {
        x,
        y,
        width: w,
        height: CTA_TOP_GAP.min(bottom.saturating_sub(y)),
    };
    y = y.saturating_add(cta_top_gap.height);
    let cta_h = CTA_BUTTON_ROWS.min(bottom.saturating_sub(y));
    let cta_row = Rect {
        x,
        y,
        width: w,
    ...
    y = y.saturating_add(cta.height);
    let hint_top_gap = Rect {
        x,
        y,
        width: w,
        height: HINT_TOP_GAP.min(bottom.saturating_sub(y)),
    };
    y = y.saturating_add(hint_top_gap.height);
    let hint = Rect {
        x,
        y,
        width: w,
        height: HINT_ROWS.min(bottom.saturating_sub(y)),
    };
...
/// Install / runtime readiness for Settings (green/red lights).
pub fn readiness_rows(home: &Path) -> Vec<SettingsRow> {
    let mut rows = vec![SettingsRow {
        kind: RowKind::Section,
        label: "Readiness".into(),
        detail: String::new(),
    }];
    for check in crate::doctor::readiness_checks(home) {
        let (dot, status) = if check.ok {
            ("●", "ok")
        } else {
            ("○", "issue")
        };
        let mut detail = format!("{dot} {status}");
        if !check.detail.is_empty() {
            detail.push_str(" · ");
            detail.push_str(&truncate_detail(&check.detail, 36));
        }
...
pub fn build_rows(config: &Config, hook_summary: &str) -> Vec<SettingsRow> {
    let mut rows = readiness_rows(&config.home);
    rows.push(SettingsRow {
        kind: RowKind::Section,
        label: "General".into(),
        detail: String::new(),
    ...
    rows.extend(group_launch_rows(&config.home));
    rows.extend(new_session_settings_rows(&config.home));
    rows.extend([
        SettingsRow {
            kind: RowKind::Section,
...
const SESSION_SHORTCUTS: &[ShortcutEntry] = &[
    ...
    ShortcutEntry {
        key: "1-9, 0, 11+",
        desc: "focus by number (sidebar)",
    },
    ShortcutEntry {
        key: "⌘1–9, ⌘0",
        desc: "focus by number (anywhere)",
    },
    ShortcutEntry {
        key: "[ / ]",
        desc: "narrower / wider sidebar (4 cols)",
    },
    ShortcutEntry {
        key: "{ / }",
        desc: "narrower / wider sidebar (10 cols)",
    },
    ShortcutEntry {
        key: "⌘⌥[ / ⌘⌥]",
        desc: "resize sidebar (Cursor anywhere)",
    },
    ShortcutEntry {
        key: "b",
        desc: "toggle sidebar rail",
    },
    ShortcutEntry {
        key: "hold d",
...
/// Up to 4 cycleable PWD group quick-launch badge slots.
pub fn group_launch_rows(home: &Path) -> Vec<SettingsRow> {
    use crate::telemetry::config::{
        group_launch_badge, group_launch_label, load_group_launch, GROUP_LAUNCH_MAX,
    };
    let agents = load_group_launch(home);
    let mut rows = vec![SettingsRow {
        kind: RowKind::Section,
        label: "PWD quick launch".into(),
        detail: String::new(),
    }];
    for slot in 0..GROUP_LAUNCH_MAX {
        let agent = agents.get(slot).map(String::as_str).unwrap_or("");
        let detail = if agent.is_empty() {
            "Off · ↵ cycle".into()
        } else {
            format!(
                "{} {} · ↵ cycle",
                group_launch_label(agent),
                group_launch_badge(agent)
            )
        };
        rows.push(SettingsRow {
            kind: RowKind::Toggle,
            label: format!("Badge {}", slot + 1),
            detail,
        });
    }
    rows
...
pub fn group_launch_slot_index(row: &SettingsRow) -> Option<usize> {
    let rest = row.label.strip_prefix("Badge ")?;
    let n: usize = rest.parse().ok()?;
    if (1..=crate::telemetry::config::GROUP_LAUNCH_MAX).contains(&n) {
        Some(n - 1)
    } else {
        None
    }
...
/// Cycle a quick-launch slot and persist. Returns true when the row was handled.
pub fn cycle_group_launch_slot(home: &Path, slot: usize) -> bool {
    use crate::telemetry::config::{
        cycle_group_launch_agent, load_group_launch, save_group_launch, GROUP_LAUNCH_MAX,
    };
    if slot >= GROUP_LAUNCH_MAX {
        return false;
    }
    let mut agents = load_group_launch(home);
    while agents.len() <= slot {
        agents.push(String::new());
    }
    let next = cycle_group_launch_agent(&agents[slot]);
    agents[slot] = next.to_string();
    while agents.last().is_some_and(|s| s.is_empty()) {
        agents.pop();
    }
    agents.retain(|s| !s.is_empty());
    let _ = save_group_launch(home, &agents);
    true
}

pub fn new_session_settings_rows(home: &Path) -> Vec<SettingsRow> {
    use crate::telemetry::config::load_new_session_preselect_agent_model;
    let preselect = load_new_session_preselect_agent_model(home);
    vec![
        SettingsRow {
            kind: RowKind::Section,
        ...
        SettingsRow {
            kind: RowKind::Toggle,
            label: "Preselect agent & model".into(),
            detail: if preselect {
                "On · ↵ toggle".into()
            } else {
                "Off · ↵ toggle".into()
...
/// Toggle new-session agent/model preselect. Returns true when the row was handled.
pub fn toggle_new_session_preselect_row(home: &Path, row: &SettingsRow) -> bool {
    if row.label != "Preselect agent & model" {
        return false;
    }
    let _ = crate::telemetry::config::toggle_new_session_preselect_agent_model(home);
    true
}
...
pub fn open_row_overlay(
    ...
    match row.label.as_str() {
        "Install update" => Some((
            SettingsOverlay::running("Upgrading sessions"),
            spawn_upgrade_job(&config.home),
        )),
        "Agent hooks" => Some((
            SettingsOverlay::running("Agent hooks"),
            spawn_hooks_job(&config.home),
        )),
        "Release notes" => {
            let lines = release_notes_lines(&config.home)?;
            Some((
                SettingsOverlay::info("Release notes", lines),
                mpsc::channel().1,
            ))
        }
        "Notes directory" => Some((
            SettingsOverlay::info(
                "Notes directory",
                vec![config.sidebar_notepad_dir().display().to_string()],
            ),
            mpsc::channel().1,
        )),
        _ => None,
...
pub fn handle_overlay_key(overlay: &mut SettingsOverlay, key: event::KeyEvent, pane: Rect) -> bool {
