...
use crate::bar::ui::{
    BG_BASE, BG_HIGHLIGHT, BG_HOVER_SELECTED, BG_PANEL, BG_SELECTED, DONE_FG, DONE_GREEN, PATH_FG,
    TEXT_PRIMARY, TEXT_SELECTED, WARM_ACCENT,
};
...
fn render_overlay(frame: &mut ratatui::Frame, pane: Rect, overlay: &SettingsOverlay) {
    ...
    let hint_y = inner
        .y
        .saturating_add(inner.height.saturating_sub(OVERLAY_HINT_ROWS));
    frame.render_widget(
...
pub fn draw_screen(
    ...
    if list_area.height > 0 {
        // Only paint rows that fit — list height is clamped when the panel is short.
    ...
    render_cta_button(frame, layout.cta, CTA_BUTTON_LABEL, panel_hover.cta);
    ...
    if layout.hint.height > 0 && layout.hint.width > 0 {
        frame.render_widget(
            Paragraph::new(Span::styled(
...
fn config_line(
    ...
    // Readiness rows use ● / ○ prefixes — color the status light green / amber.
    let readiness_fg = if detail.starts_with('●') {
        Some(DONE_FG)
    ...
    let detail_style = if let Some(fg) = readiness_fg {
        if selected || hovered {
            Style::default()
                .fg(fg)
                .bg(row_bg)
