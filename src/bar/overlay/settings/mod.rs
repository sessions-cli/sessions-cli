...
use state::{
    ...
    panel_content_height, row_opens_overlay, sync_panel_mouse_cursor, OverlayMsg, RowKind,
    SettingsOverlay,
};
...
mod tests {
    ...
    use state::{
        build_list_layout, list_row_rects, settings_layout, update_rows, CTA_TOP_GAP, HINT_TOP_GAP,
        ListLine, OverlayMsg, OVERLAY_HINT_ROWS, PanelTargets,
    };
    use super::*;
    ...
    fn list_row_rects_skip_sections_and_gaps() {
        ...
        let hook_summary = hooks::integrations_summary(&config.home);
        let rows = build_rows(&config, &hook_summary);
        let list_area = Rect::new(5, 10, 40, 30);
        ...
        let completion_y = rects
            ...
            .map(|(_, rect)| rect.y)
            .unwrap();
        assert_eq!(notes_y, list_area.y + 2);
        assert!(
