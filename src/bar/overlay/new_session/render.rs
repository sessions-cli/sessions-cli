...
use super::state::{union_rect, Focus, LaunchMode, NewSessionState, PanelHover, PathGhostHint, WorkspacePopupEntry, WorkspacePopupKind, CLOSE_BUTTON_COLS, CLOSE_BUTTON_LABEL, DIRECTORY_DISPLAY_INITIAL, DIRECTORY_DISPLAY_PAGE, EXISTING_WORKSPACES_HEADER, FIELD_INNER_HEIGHT, MAX_DROPDOWN_VISIBLE, NEW_WORKSPACE_HEADER, PATH_SUGGESTIONS_HEADER, PROMPT_INNER_HEIGHT, SECTION_GAP, TITLE_ROWS, WORKSPACE_HEADER_ROWS};
use crate::agents;
...
use crate::bar::ui::{BG_BASE, BG_HIGHLIGHT, BG_HOVER_SELECTED, BG_PANEL, BG_SELECTED, CLOSE_HOVER_FG, PATH_FG, TEXT_PRIMARY, TEXT_SELECTED};
...
pub(crate) fn modal_content_height() -> u16 {
    // Full form height (for tests and default sizing)
...
pub(crate) fn modal_content_height_for_state(state: &NewSessionState) -> u16 {
    ...
    let mut h = TITLE_ROWS + 1 + 1 + workspace_h + SECTION_GAP + 1 + agent_h + SECTION_GAP;
    if !is_console {
        let model_h = dropdown_field_height();
        let prompt_h = field_block_height(PROMPT_INNER_HEIGHT);
...
pub(crate) fn new_session_layout(pane: Rect, state: &NewSessionState) -> NewSessionLayout {
    ...
    let top_margin = if pane.height > form_height + 4 { (pane.height.saturating_sub(form_height)) / 2 } else { 2 };
    let form_y = pane.y.saturating_add(top_margin);
    NewSessionLayout { form: Rect { x: form_x, y: form_y, width: form_width, height: form_height } }
}
...
fn render_close_button(frame: &mut ratatui::Frame, row: Rect, hovered: bool) {
    ...
    let label_area = Rect {
        x: row.x.saturating_add(row.width.saturating_sub(CLOSE_BUTTON_COLS)),
...
pub fn draw_screen(
    ...
    let workspace_h = dropdown_field_height();
    let agent_h = dropdown_field_height();
    let model_h = if is_console { 0 } else { dropdown_field_height() };
    let prompt_h = if is_console { 0 } else { field_block_height(PROMPT_INNER_HEIGHT) };
    let button_h = field_block_height(FIELD_INNER_HEIGHT);
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(SECTION_GAP),
            Constraint::Length(if is_console { 0 } else { 1 }),
            Constraint::Length(model_h),
            Constraint::Length(model_gap),
            Constraint::Length(if is_console { 0 } else { 1 }),
    ...
    // is_console already computed earlier for layout
    ...
    let (workspace_value_area, _) = split_field_area(chunks[2], workspace_focused);
    ...
    let agent_popup_rect = if agent_open {
        compute_dropdown_menu_rect(chunks[5], agent_labels.len(), state.agent_idx, true, area)
    } else {
        Rect::default()
    };
    let model_popup_rect = if model_open {
        compute_dropdown_menu_rect(chunks[8], model_labels.len(), state.model_idx, true, area)
    } else {
        Rect::default()
    };
    ...
    if !is_console && !model_open {
        if let Some(visible) = render_if_visible(chunks[8], viewport) {
            if !covered(visible) {
                let (value_area, _) = split_field_area(visible, false);
                render_dropdown_collapsed(
                    frame,
    ...
    let hint = if let Some(ref e) = path_err {
        ...
        match state.focus {
            Focus::Workspace if state.is_typing_path() => {
                "↑↓ pick · ←→ edit path · Tab complete · ↵ confirm".into()
            }
            Focus::Workspace => {
                "↵ confirm · Tab cycle · type to search · pick active session or directory"
                    .into()
            }
            Focus::Prompt => {
                "↵ newline · ⌘A select all · Tab buttons".into()
            }
    ...
    let (agent_value_area, _) = split_field_area(chunks[5], agent_open);
    let model_value_area = if !is_console {
        split_field_area(chunks[8], model_open).0
    } else {
        Rect::default()
    };
    ...
    // Cursor indicator in the directory (session path) field so the user sees
    // they can type immediately when the new-session pane opens (and while the
    // current header value is the live user-edited path). We show it for the
    // "New session" path row (the editable header), whether the value is the
    // default ~/ starter or a custom typed path (valid or not yet).
    if workspace_focused && state.is_typing_path() {
        ...
        if on_path_row {
            ...
            if anchor.width > 2 && anchor.height > 0 {
                ...
                let max_list_rows = frame_bottom
                    .saturating_sub(anchor.y + 2 + WORKSPACE_HEADER_ROWS) as usize;
                ...
                let menu_height = WORKSPACE_HEADER_ROWS
                    ...
                    .saturating_add(2)
                    .max(anchor.height);
                let menu = Rect { x: anchor.x, y: anchor.y, width: anchor.width, height: menu_height };
                ...
                if inner.height > 0 && inner.width > 2 {
                    ...
                    if pos.x < area.x.saturating_add(area.width) && pos.y < area.y.saturating_add(area.height) {
                        frame.set_cursor_position(pos);
                    }
                }
            }
    ...
    let workspace_target = if workspace_focused && workspace_popup.width > 0 {
        union_rect(chunks[1], workspace_popup)
    } else {
        union_rect(chunks[1], chunks[2])
    };
    let agent_target = if agent_open {
        union_rect(chunks[4], agent_menu)
    } else {
        union_rect(chunks[4], chunks[5])
    };
    let model_target = if model_open {
        union_rect(chunks[7], model_menu)
    } else {
        Rect::default()
    };
    ...
    ClickTargets {
        ...
        workspace_field: chunks[2],
        workspace_popup,
        agent: agent_target,
        agent_field: if agent_open { agent_menu } else { chunks[5] },
        ...
        model: model_target,
        model_field: if model_open { model_menu } else { Rect::default() },
        ...
        prompt_field: if !is_console { chunks[11] } else { Rect::default() },
        foreground_button,
        background_button,
        close: close_target,
    }
...
pub(crate) fn workspace_list_window(count: usize, selected: usize, max_visible_rows: usize) -> (usize, usize) {
    if count == 0 || max_visible_rows == 0 {
        return (0, 0);
    }
...
pub(in crate::bar::overlay::new_session) fn workspace_popup_header_click(popup_area: Rect, col: u16, row: u16) -> bool {
...
fn workspace_popup_row_label(entry: &WorkspacePopupEntry) -> String {
    match entry.kind {
        ...
        WorkspacePopupKind::Existing(_) | WorkspacePopupKind::Path => {
            ...
            // agent/group labels in cwd (e.g. "G downloads", "O Superflip", "O cursor").
...
pub(in crate::bar::overlay::new_session) fn point_in_prompt(col: u16, row: u16, targets: &ClickTargets) -> bool {
