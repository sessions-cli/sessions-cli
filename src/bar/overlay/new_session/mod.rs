...
pub use launch::preset_workspace_for_new_session;
...
impl NewSessionPanel {
    ...
    pub fn handle_key(&mut self, config: &Config, key: event::KeyEvent) -> Result<NewSessionAction> {
    ...
    pub fn handle_mouse(
        ...
        handle_mouse_event(mouse, &self.targets, &mut self.hover, &mut self.state, config)
    }
...
mod tests {
    ...
    use super::render::{
        ...
        submit_button_backdrop, workspace_list_window, workspace_popup_row_from_mouse, BG_FIELD,
        ClickTargets,
    };
    use super::state::*;
    use super::*;
    ...
    fn prompt_editing_inserts_at_cursor_and_supports_newlines() {
        ...
        assert_eq!(state.prompt_selection, Some((0, 5)));
        assert_eq!(
            notepad::selected_text(&state.prompt, 0, 5),
            "hi!\nx"
        );
    }
    ...
    fn prompt_drag_updates_selection() {
        ...
        let field = Rect::new(0, 0, 24, field_block_height(PROMPT_INNER_HEIGHT));
        let inner = prompt_field_inner(field);
        state.handle_prompt_body_click(field, inner.x.saturating_add(1), inner.y, state.prompt_field_width);
    ...
    fn focus_tab_order_cycles_through_form() {
        assert_eq!(focus_next(Focus::Workspace), Focus::Agent);
        assert_eq!(focus_next(Focus::Agent), Focus::Model);
        assert_eq!(focus_next(Focus::Model), Focus::Prompt);
        assert_eq!(focus_next(Focus::Prompt), Focus::ForegroundButton);
        assert_eq!(focus_next(Focus::BackgroundButton), Focus::Workspace);
        assert_eq!(focus_prev(Focus::Workspace), Focus::BackgroundButton);
        assert_eq!(focus_prev(Focus::Model), Focus::Agent);
        assert_eq!(focus_prev(Focus::Prompt), Focus::Model);
    ...
    fn path_completions_suggest_projects_children() {
        ...
        let completions = test_directory_index().completions_for_input("~/projects/sessions");
        assert!(
            completions
                .iter()
    ...
    fn confirmed_custom_path_survives_focus_blur() {
        ...
        let personal = format!("{home}/projects/personal-website");
        ...
        state.workspace_path_input = "~/projects/personal-website".into();
        state.workspace_user_editing = true;
        state.workspace_popup_highlight = state.first_popup_highlight_for_query();
        assert!(state.confirm_workspace_enter());
        assert_eq!(state.workspace_path_input, "~/projects/personal-website");
        assert_eq!(state.workspace_header_display(), "~/projects/personal-website");
        state.set_focus(Focus::Agent);
        assert_eq!(state.workspace_committed_display(), "~/projects/personal-website");
        assert!(state.workspace_confirmed);
    }
    ...
    fn custom_path_header_ignores_stale_existing_highlight() {
        ...
        let mut state = test_state_with_workspace("~/projects/sessions-cli", &sessions);
        state.select_path_label("~/projects/personal-website".into(), false);
    ...
    fn tab_complete_workspace_extends_typed_row() {
        ...
        assert!(
            state.workspace_path_input.starts_with("~/projects/sessions"),
    ...
    fn browse_mode_defaults_to_most_popular_session_path() {
        ...
        let label = entries
            ...
            .and_then(|entry| match entry.kind {
                WorkspacePopupKind::Existing(idx) => state.workspaces.get(idx).map(|w| w.label.clone()),
                _ => None,
            })
