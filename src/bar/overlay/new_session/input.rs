...
use super::render::{draw_screen, dropdown_popup_click_index, point_in_prompt, sync_panel_mouse_cursor, workspace_popup_click_index, workspace_popup_header_click, workspace_popup_row_from_mouse, ClickTargets};
use super::state::{path_entry_char, Focus, LaunchMode, LaunchOutcome, NewSessionState, PanelHover};
use crate::agents;
...
pub fn handle_key(
    ...
    if key.code == KeyCode::Enter && has_paste_modifier(key.modifiers) && state.focus != Focus::Prompt {
        if state.launch(config, LaunchMode::Open)? == LaunchOutcome::Opened {
            return Ok(NewSessionAction::Launched);
        }
        return Ok(NewSessionAction::Unchanged);
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(100))? {
            match event::read()? {
                ...
                Event::Mouse(mouse) => match handle_mouse_event(
                    mouse,
                    &click_targets,
                    panel_hover,
                    state,
