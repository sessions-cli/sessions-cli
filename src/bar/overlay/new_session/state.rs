...
impl NewSessionState {
    pub fn new(config: &Config) -> Result<Self> {
        ...
        let workspace_idx = preset_label
            ...
            .unwrap_or(0);
        let (mut workspace_path_input, mut workspace_user_editing) =
    ...
    pub(in crate::bar::overlay::new_session) fn apply_draft(&mut self, draft: NewSessionDraft) {
    ...
    pub(in crate::bar::overlay::new_session) fn  selected_workspace(&self) -> Option<&WorkspaceChoice> {
    ...
    pub(in crate::bar::overlay::new_session) fn  directory_completions(&self) -> Vec<(String, String)> {
        ...
        if self.workspace_path_input.trim().is_empty() {
            ...
            for (label, cwd) in self.workspace_usage.closed_suggestions(
                ...
                if !out.iter().any(|(existing, path)| existing == &label || path == &cwd) {
                    out.push((label, cwd));
                }
            }
    ...
    pub(in crate::bar::overlay::new_session) fn  sort_directory_completions(&self, completions: &mut [(String, String)]) {
    ...
    pub(in crate::bar::overlay::new_session) fn  maybe_expand_directory_list(&mut self) {
        ...
        let last_directory_idx = entries
            ...
            .enumerate()
            .filter(|(_, entry)| {
                entry.kind == WorkspacePopupKind::Path && entry.cwd.is_some()
            })
            .map(|(idx, _)| idx)
            .last();
    ...
    pub(in crate::bar::overlay::new_session) fn  build_workspace_popup(&self) -> Vec<WorkspacePopupEntry> {
        ...
        if typing {
            ...
            if !typed.is_empty() {
                // If the typed value currently resolves to a real directory, represent
                // it using the normalized label and real cwd so it behaves like other
                // suggestions (and Enter will accept it without "not a directory").
                let (label, cwd) = match expand_workspace_path(typed) {
                    Ok(resolved) => {
                        let home = std::env::var("HOME").unwrap_or_default();
                        let is_home = resolved == home || resolved.trim_end_matches('/') == home;
                        let l = if is_home && (typed == "~" || typed == "~/") {
                            // Default the search bar to ~/ — user must backspace to remove.
                            "~/".to_string()
                        } else {
                            format_tilde_path(&resolved)
                        };
                        (l, Some(resolved))
                    }
    ...
    pub(in crate::bar::overlay::new_session) fn  sync_popup_highlight_to_path_label(&mut self, label: &str) {
        let entries = self.build_workspace_popup();
    ...
    pub(in crate::bar::overlay::new_session) fn select_path_label(&mut self, label: String, from_user_edit: bool) {
    ...
    pub(in crate::bar::overlay::new_session) fn  confirm_workspace_selection(&mut self, show_errors: bool) -> bool {
        self.status.clear();
    ...
    pub(in crate::bar::overlay::new_session) fn  on_workspace_typed_row(&self) -> bool {
        ...
        let entries = self.build_workspace_popup();
        entries.get(self.workspace_popup_highlight).is_some_and(|entry| {
            entry.kind == WorkspacePopupKind::Path
                && entry.label.trim() == self.workspace_path_input.trim()
        })
    }

    pub(in crate::bar::overlay::new_session) fn  path_completion_candidates(&self) -> Vec<(String, String)> {
    ...
    pub(in crate::bar::overlay::new_session) fn  workspace_path_ghost_hint(&self) -> Option<PathGhostHint> {
    ...
    pub(in crate::bar::overlay::new_session) fn apply_workspace_path_completion(&mut self, completion: &(String, String)) {
    ...
    pub(in crate::bar::overlay::new_session) fn on_workspace_backspace(&mut self) {
        ...
        let next_byte = self
            ...
            .map(|(idx, _)| idx)
            .unwrap_or(self.workspace_path_input.len());
        self.workspace_path_input.replace_range(byte_idx..next_byte, "");
        self.workspace_path_cursor = cursor - 1;
    ...
    pub(in crate::bar::overlay::new_session) fn  ranked_workspaces(&self) -> Vec<(usize, &WorkspaceChoice)> {
        let mut ranked: Vec<(usize, &WorkspaceChoice, (i64, i64))> = self
            .workspaces
            .iter()
            .enumerate()
            .map(|(idx, workspace)| {
                let score = self.workspace_usage.rank_score(
                    &workspace.cwd,
                    None,
                    self.rank_mode,
                );
        ...
        ranked.sort_by(|left, right| {
            right.2.cmp(&left.2).then_with(|| left.1.label.cmp(&right.1.label))
        });
        ranked
            .into_iter()
    ...
    pub(in crate::bar::overlay::new_session) fn  default_workspace_popup_highlight(&self) -> usize {
        let entries = self.build_workspace_popup();
        if self.workspace_path_input.trim().is_empty() {
            if let Some((idx, _)) = entries.iter().enumerate().find(|(_, entry)| {
                matches!(entry.kind, WorkspacePopupKind::Existing(_))
            }) {
                return idx;
            }
        }
    ...
    pub(in crate::bar::overlay::new_session) fn  resolve_workspace(&self) -> Result<(String, String)> {
        if self.uses_custom_path() {
    ...
    pub(in crate::bar::overlay::new_session) fn  model_id_for_agent(&self, agent: &AgentEntry) -> &str {
        self.defaults
            .agent_models
            .get(agent.id)
            .map(String::as_str)
    ...
    pub(in crate::bar::overlay::new_session) fn workspace_popup_selectable_indices(entries: &[WorkspacePopupEntry]) -> Vec<usize> {
        entries
            .iter()
            .enumerate()
    ...
    pub(in crate::bar::overlay::new_session) fn  cycle_agent(&mut self, delta: i32) {
        ...
        self.sync_model_idx();
        if self.selected_agent().id == "console" && matches!(self.focus, Focus::Model | Focus::Prompt) {
            self.focus = Focus::ForegroundButton;
        }
    }
    ...
    pub(in crate::bar::overlay::new_session) fn  next_focus(&self) -> Focus {
        match self.focus {
            ...
            Focus::BackgroundButton => Focus::Workspace,
        }
    }

    pub(in crate::bar::overlay::new_session) fn  prev_focus(&self) -> Focus {
        match self.focus {
            Focus::Workspace => Focus::BackgroundButton,
            Focus::Agent => Focus::Workspace,
    ...
    pub(in crate::bar::overlay::new_session) fn  prompt_content_width(&self, inner_width: u16) -> usize {
        inner_width.saturating_sub(2) as usize
    }

    pub(in crate::bar::overlay::new_session) fn  sync_prompt_scroll(&mut self, content_width: usize) {
        let cursor_line =
    ...
    pub(in crate::bar::overlay::new_session) fn  scroll_prompt_lines(&mut self, delta: i32, content_width: usize) {
        let line_count =
            notepad::wrapped_display_lines(&self.prompt, content_width).len();
        let viewport_rows = PROMPT_INNER_HEIGHT as usize;
        let max_scroll = line_count.saturating_sub(viewport_rows);
        let next = (self.prompt_scroll as i32 + delta)
    ...
    pub(in crate::bar::overlay::new_session) fn  prompt_delete_selection(&mut self, content_width: usize) -> bool {
    ...
    pub(in crate::bar::overlay::new_session) fn  insert_prompt_char(&mut self, ch: char, content_width: usize) {
    ...
    pub(in crate::bar::overlay::new_session) fn  insert_prompt_str(&mut self, text: &str, content_width: usize) {
        if text.is_empty() {
            return;
        }
        self.prompt_delete_selection(content_width);
    ...
    pub(in crate::bar::overlay::new_session) fn  prompt_forward_delete(&mut self, content_width: usize) {
        let cursor = notepad::clamp_cursor(&self.prompt, self.prompt_cursor);
    ...
    pub(in crate::bar::overlay::new_session) fn  move_prompt_cursor(&mut self, delta: i32, content_width: usize) {
    ...
    pub(in crate::bar::overlay::new_session) fn  move_prompt_cursor_vertical(&mut self, delta: i32, content_width: usize) {
        ...
        let target_line =
            (display_line as i32 + delta).clamp(0, wrapped.len().saturating_sub(1) as i32)
                as usize;
    ...
    pub(in crate::bar::overlay::new_session) fn  cut_prompt_selection(&mut self, content_width: usize) -> bool {
        if !self
            .prompt_selection
    ...
    pub(in crate::bar::overlay::new_session) fn  focus_prompt_at_cursor(&mut self, cursor: Option<usize>, content_width: usize) {
    ...
    pub(in crate::bar::overlay::new_session) fn  focus_prompt_from_keyboard(&mut self, content_width: usize) {
    ...
    pub(in crate::bar::overlay::new_session) fn  prompt_register_click(&mut self, column: u16, row: u16) -> u8 {
...
pub(in crate::bar::overlay::new_session) fn save_defaults(config: &Config, defaults: &NewSessionDefaults) -> Result<()> {
    let path = config.new_session_defaults_path();
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
pub(in crate::bar::overlay::new_session) fn load_workspaces(
    ...
    workspaces.into_iter().map(|(workspace, _)| workspace).collect()
}
...
pub(in crate::bar::overlay::new_session) fn expand_tilde_path(path: &str) -> Result<String> {
    let trimmed = path.trim();
    if trimmed.is_empty() {
        anyhow::bail!("enter a workspace path");
    }
    let home = std::env::var("HOME").context("HOME is not set")?;
    crate::bar::directory_discovery::expand_tilde(trimmed, &home)
        .ok_or_else(|| anyhow::anyhow!("could not expand path"))
}

pub(in crate::bar::overlay::new_session) fn expand_workspace_path(input: &str) -> Result<String> {
    let expanded = expand_tilde_path(input)?;
    let path = Path::new(&expanded);
    if !path.is_dir() {
        if let Some(parent) = path.parent() {
            if !parent.exists() || !parent.is_dir() {
                anyhow::bail!("parent directory does not exist: {}", input.trim());
            }
        }
        anyhow::bail!("not a directory: {}", input.trim());
    }
...
pub(in crate::bar::overlay::new_session) fn completion_label_with_dir_slash(completion: &(String, String)) -> String {
...
pub(in crate::bar::overlay::new_session) fn longest_path_completion(input: &str, completions: &[(String, String)]) -> Option<String> {
    if completions.is_empty() {
        return None;
    }
    if completions.len() == 1 {
    ...
    // Support bare input (e.g. "pic") against labels like "~/Pictures" or "~/proj/pics".
    // Compute common prefix on the basenames and reconstruct a ~ label if it extends the query.
...
fn prompt_field_inner(area: Rect) -> Rect {
    ...
    Block::default().borders(Borders::ALL).style(Style::default()).inner(area)
}

fn prompt_cursor_from_mouse(area: Rect, col: u16, row: u16, text: &str, scroll: usize, content_width: usize) -> Option<usize> {
    let inner = prompt_field_inner(area);
    if inner.width == 0 || inner.height == 0 || !point_in_rect(col, row, inner) { return None; }
    ...
    Some(notepad::clamp_cursor(text, line.start.saturating_add(col_in_line)))
}

fn prompt_selection_cursor_from_mouse(area: Rect, col: u16, row: u16, text: &str, scroll: usize, content_width: usize) -> Option<usize> {
    if let Some(cursor) = prompt_cursor_from_mouse(area, col, row, text, scroll, content_width) { return Some(cursor); }
    let inner = prompt_field_inner(area);
    if inner.width == 0 || inner.height == 0 || !point_in_rect(col, row, area) { return None; }
    let wrapped = notepad::wrapped_display_lines(text, content_width);
    if row < inner.y { let first = wrapped.first()?; return Some(first.start); }
    if row >= inner.y.saturating_add(inner.height) { let last = wrapped.last()?; return Some(last.start.saturating_add(last.text.chars().count())); }
    let rel_row = if row < inner.y.saturating_add(inner.height / 2) { 0usize } else { PROMPT_INNER_HEIGHT.saturating_sub(1) as usize };
    let display_line_idx = scroll.saturating_add(rel_row);
    let line = wrapped.get(display_line_idx)?;
    let rel_col = col.saturating_sub(inner.x.saturating_add(1)) as usize;
    let col_in_line = if col < inner.x.saturating_add(1) { 0 } else { rel_col.min(line.text.chars().count()) };
    Some(notepad::clamp_cursor(text, line.start.saturating_add(col_in_line)))
}
