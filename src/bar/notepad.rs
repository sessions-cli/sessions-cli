...
/// Markdown task-list line: `- [ ] item` / `* [x] item`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MdTaskListLine {
    pub indent: usize,
    pub marker: char,
...
/// Bracket span within a task-list line.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MdCheckboxSpan {
    pub bracket_start: usize,
    pub bracket_end: usize,
    pub checked: bool,
}

/// Two-column hit target: empty slot when unchecked, solid DONE_FG block when checked.
pub const MD_CHECKBOX_UNCHECKED_GLYPH: &str = "  ";
pub const MD_CHECKBOX_CHECKED_GLYPH: &str = "██";
pub const MD_CHECKBOX_DISPLAY_WIDTH: usize = 2;

pub fn format_md_checkbox_checked_glyph() -> String {
    MD_CHECKBOX_CHECKED_GLYPH.to_string()
}

fn md_task_list_display_delta(task: &MdTaskListLine) -> usize {
    task.bracket_end
        .saturating_sub(task.indent)
        .saturating_sub(MD_CHECKBOX_DISPLAY_WIDTH)
}

pub fn md_task_list_line(line: &str) -> Option<MdTaskListLine> {
    let chars: Vec<char> = line.chars().collect();
    let mut indent = 0usize;
    while indent < chars.len() && chars[indent].is_whitespace() {
        indent += 1;
    }
    if indent >= chars.len() || !matches!(chars[indent], '-' | '*' | '+') {
        return None;
    }
...
pub fn md_checkbox_in_line(line: &str) -> Option<MdCheckboxSpan> {
    md_task_list_line(line).map(|task| MdCheckboxSpan {
        bracket_start: task.bracket_start,
        bracket_end: task.bracket_end,
        checked: task.checked,
    })
}

pub fn md_task_list_line_is_empty(line: &str) -> bool {
    let Some(task) = md_task_list_line(line) else {
        return false;
    };
    line.chars()
        .skip(task.bracket_end)
        .collect::<String>()
        .trim()
        .is_empty()
}

pub fn md_task_list_continue_prefix(line: &str) -> Option<String> {
    let task = md_task_list_line(line)?;
    let leading: String = line.chars().take(task.indent).collect();
    Some(format!("{leading}{} [ ] ", task.marker))
}

pub fn cursor_at_logical_line_end(text: &str, cursor: usize) -> bool {
    let (line, col) = cursor_line_col(text, cursor);
    col >= line_len_at(text, line)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MdTaskEnterAction {
    PlainNewline,
    ContinueList { prefix: String },
    ExitEmptyList { line_start: usize },
}

pub fn md_task_enter_action(text: &str, cursor: usize) -> MdTaskEnterAction {
    if !cursor_at_logical_line_end(text, cursor) {
        return MdTaskEnterAction::PlainNewline;
    }
    let (line_idx, _) = cursor_line_col(text, cursor);
    let line_start = cursor_from_line_col(text, line_idx, 0);
    let line_len = line_len_at(text, line_idx);
    let line: String = text.chars().skip(line_start).take(line_len).collect();
    if md_task_list_line(&line).is_none() {
        return MdTaskEnterAction::PlainNewline;
    }
    if md_task_list_line_is_empty(&line) {
        return MdTaskEnterAction::ExitEmptyList { line_start };
    }
    if let Some(prefix) = md_task_list_continue_prefix(&line) {
        return MdTaskEnterAction::ContinueList { prefix };
    }
    MdTaskEnterAction::PlainNewline
}

pub fn md_checkbox_show_glyph(
    line: &str,
    line_start: usize,
    selection: Option<(usize, usize)>,
    literal_edit_bracket: Option<usize>,
) -> bool {
    let Some(cb) = md_checkbox_in_line(line) else {
        return false;
    };
    let abs_start = line_start + cb.bracket_start;
    let abs_end = line_start + cb.bracket_end;
    if literal_edit_bracket == Some(abs_start) {
        return false;
    }
    if let Some((sel_start, sel_end)) = selection {
        if sel_start < sel_end && sel_start < abs_end && sel_end > abs_start {
            return false;
        }
    }
    true
...
pub fn display_col_for_raw_col_in_line(
    line: &str,
    raw_col: usize,
    line_start: usize,
    selection: Option<(usize, usize)>,
    ...
    if !md_checkbox_show_glyph(line, line_start, selection, literal_edit_bracket) {
        return raw_col;
    }
    let Some(task) = md_task_list_line(line) else {
        return raw_col;
    };
    if raw_col < task.indent {
        return raw_col;
    }
    if raw_col < task.bracket_end {
        return task.indent;
    }
    raw_col.saturating_sub(md_task_list_display_delta(&task))
}

pub fn raw_col_for_display_col_in_line(
    line: &str,
    display_col: usize,
    line_start: usize,
    selection: Option<(usize, usize)>,
    ...
    if display_col < task.indent {
        return display_col;
    }
    if display_col < task.indent.saturating_add(MD_CHECKBOX_DISPLAY_WIDTH) {
        return task.bracket_start;
    }
    display_col.saturating_add(md_task_list_display_delta(&task))
}

pub fn md_checkbox_terminal_cursor_hidden(
    text: &str,
    cursor: usize,
    selection: Option<(usize, usize)>,
    literal_edit_bracket: Option<usize>,
    ...
    let (line_idx, col) = cursor_line_col(text, cursor);
    let line_start = cursor_from_line_col(text, line_idx, 0);
    let line_len = line_len_at(text, line_idx);
    let line: String = text.chars().skip(line_start).take(line_len).collect();
    let Some(task) = md_task_list_line(&line) else {
    ...
    col >= task.bracket_start && col <= task.bracket_end
}

pub fn md_checkbox_display_hit(
    line: &str,
    display_col: usize,
    line_start: usize,
    selection: Option<(usize, usize)>,
    ...
    md_checkbox_show_glyph(line, line_start, selection, literal_edit_bracket)
        && display_col >= task.indent
        && display_col < task.indent.saturating_add(MD_CHECKBOX_DISPLAY_WIDTH)
}

/// Backspace at the start of task content demotes the rendered checkbox to `- [ ]` markdown.
pub fn md_checkbox_demote_backspace(
    text: &str,
    cursor: usize,
    literal_edit_bracket: Option<usize>,
) -> Option<usize> {
    if literal_edit_bracket.is_some() {
        return None;
    }
    let cursor = clamp_cursor(text, cursor);
    let (line_idx, col) = cursor_line_col(text, cursor);
    ...
    let line: String = text.chars().skip(line_start).take(line_len).collect();
    let task = md_task_list_line(&line)?;
    if col != task.bracket_end {
        return None;
    }
    Some(line_start + task.bracket_start)
}

pub fn md_checkbox_at(text: &str, cursor: usize) -> Option<usize> {
    let cursor = clamp_cursor(text, cursor);
    let (line_idx, col) = cursor_line_col(text, cursor);
    let line_start = cursor_from_line_col(text, line_idx, 0);
    let line_len = line_len_at(text, line_idx);
    let line: String = text.chars().skip(line_start).take(line_len).collect();
    let cb = md_checkbox_in_line(&line)?;
    (col >= cb.bracket_start && col < cb.bracket_end).then_some(line_start + cb.bracket_start)
}

pub fn toggle_md_checkbox_at(text: &mut String, bracket_start: usize) -> bool {
    let bracket_start = clamp_cursor(text, bracket_start);
    let chars: Vec<char> = text.chars().collect();
    if bracket_start + 2 >= chars.len()
        || chars[bracket_start] != '['
        || chars[bracket_start + 2] != ']'
    {
        return false;
    }
    let mark = chars[bracket_start + 1];
    let checked = matches!(mark, 'x' | 'X');
    if mark != ' ' && !checked {
        return false;
    }
    let new_mark = if checked { ' ' } else { 'x' };
    let byte_start = text
        .char_indices()
        .nth(bracket_start + 1)
        .map(|(idx, _)| idx)
    ...
    let byte_end = text
        ...
        .map(|(idx, _)| idx)
        .unwrap_or(text.len());
    text.replace_range(byte_start..byte_end, &new_mark.to_string());
    true
}
...
pub fn url_at(text: &str, cursor: usize) -> Option<String> {
    ...
    let end_byte = bytes.get(end).map(|(idx, _)| *idx).unwrap_or(text.len());
...
pub fn delete_note_file(config: &Config, note_id: &str) -> Result<()> {
    ...
    if path.exists() {
        fs::remove_file(&path).with_context(|| format!("remove note file {}", path.display()))?;
    }
    Ok(())
}
...
fn maybe_backup_notepad_dir(config: &Config, next: &SidebarNotepad) -> Result<()> {
    ...
    atomic_write_json(
        &prefs_backup_path(&prefs_path),
        &NotepadPrefs::from(&current),
    )
}
...
fn push_hard_broken_word(out: &mut Vec<DisplayLine>, word_start: usize, word: &str, width: usize) {
...
mod tests {
    ...
    #[test]
    fn md_checkbox_in_line_parses_task_list_markers() {
        let cb = md_checkbox_in_line("- [ ] task").expect("unchecked");
        assert_eq!(cb.bracket_start, 2);
        assert!(!cb.checked);
        let cb = md_checkbox_in_line("* [x] done").expect("checked");
        assert!(cb.checked);
        assert!(md_checkbox_in_line("- [] bad").is_none());
        assert!(md_checkbox_in_line("plain line").is_none());
    }

    #[test]
    fn md_checkbox_display_col_mapping_round_trips() {
        let line = "- [ ] foo";
        let line_start = 0;
        let selection = None;
        for raw in 0..line.chars().count() {
            let display = display_col_for_raw_col_in_line(line, raw, line_start, selection, None);
            let back = raw_col_for_display_col_in_line(line, display, line_start, selection, None);
            if raw < 5 {
                assert_eq!(display, 0, "raw {raw}");
                assert_eq!(back, 2, "raw {raw}");
            } else {
                assert_eq!(display, raw - 3, "raw {raw}");
                assert_eq!(back, raw, "raw {raw}");
            }
        }
    }

    #[test]
    fn md_checkbox_edit_mode_uses_literal_columns() {
        let line = "- [ ] foo";
        let literal_edit = Some(2);
        assert!(!md_checkbox_show_glyph(line, 0, None, literal_edit));
        assert_eq!(
            display_col_for_raw_col_in_line(line, 3, 0, None, literal_edit),
            3
        );
    }

    #[test]
    fn md_checkbox_demote_backspace_at_task_content_start() {
        let text = "- [ ] task";
        let cursor = 5;
        assert_eq!(md_checkbox_demote_backspace(text, cursor, None), Some(2));
        assert!(md_checkbox_demote_backspace(text, cursor, Some(2)).is_none());
    }

    #[test]
    fn md_task_enter_continues_non_empty_checkbox_line() {
        let text = "- [ ] one";
        match md_task_enter_action(text, text.chars().count()) {
            MdTaskEnterAction::ContinueList { prefix } => assert_eq!(prefix, "- [ ] "),
            other => panic!("expected continue, got {other:?}"),
        }
    }

    #[test]
    fn md_task_enter_exits_empty_checkbox_line() {
        let text = "head\n- [ ] ";
        let cursor = text.chars().count();
        match md_task_enter_action(text, cursor) {
            MdTaskEnterAction::ExitEmptyList { line_start } => assert_eq!(line_start, 5),
            other => panic!("expected exit, got {other:?}"),
        }
    }

    #[test]
    fn md_task_enter_plain_when_not_at_line_end() {
        let text = "- [ ] one";
        assert!(matches!(
            md_task_enter_action(text, 2),
            MdTaskEnterAction::PlainNewline
        ));
    }

    #[test]
    fn md_checkbox_checked_glyph_is_solid_green_block() {
        let glyph = format_md_checkbox_checked_glyph();
        assert_eq!(glyph, MD_CHECKBOX_CHECKED_GLYPH);
        assert_eq!(glyph.chars().count(), MD_CHECKBOX_DISPLAY_WIDTH);
        assert!(!glyph.contains('⬜'));
        assert!(!glyph.contains('✅'));
        assert_ne!(glyph, MD_CHECKBOX_UNCHECKED_GLYPH);
    }

    #[test]
    fn md_checkbox_at_and_toggle() {
        let text = "todo\n- [ ] one\n- [x] two";
        let line1_start = cursor_from_line_col(text, 1, 0);
        assert_eq!(md_checkbox_at(text, line1_start + 2), Some(line1_start + 2));
        assert_eq!(md_checkbox_at(text, line1_start + 4), Some(line1_start + 2));
        assert!(md_checkbox_at(text, line1_start + 6).is_none());

        let mut edited = text.to_string();
        let hit = md_checkbox_at(&edited, line1_start + 3).unwrap();
        assert!(toggle_md_checkbox_at(&mut edited, hit));
        assert!(edited.contains("- [x] one"));
        let line2_start = cursor_from_line_col(&edited, 2, 0);
        let hit = md_checkbox_at(&edited, line2_start + 3).unwrap();
        assert!(toggle_md_checkbox_at(&mut edited, hit));
