...
use targets::{
    enrich_notify_targets, resolve_agent_session_from_pane, resolve_notify_targets,
};
...
fn run_notify_inner(event: &str, payload: Option<&str>, use_stdin: bool) -> Result<()> {
    ...
    let mut agent_session_id = crate::agents::resolve_session_id_from_env().map(|(sid, _)| sid);
    ...
    if agent_session_id.is_none() {
        ...
        if let Some(sid) = payload_sid {
            agent_session_id = Some(sid.to_string());
            session_env = load_session_env(&config.session_env_path(sid));
            (tmux_pane_id, tmux_session, kitty_window_id) = resolve_notify_targets(
                event,
                &session_env,
                shell_pane.clone(),
