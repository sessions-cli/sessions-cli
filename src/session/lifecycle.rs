...
use crate::pty::{agent_from_command, format_session_title, CONSOLE_LABEL};
use crate::session::managed::{new_sessions_session_id, ManagedLaunchRecord};
use crate::session::manifest::{
    append_entry, load_manifest, manifest_entry_for_ssn, mark_entry_closed, reopen_manifest_entry,
...
impl LaunchSpec {
    pub fn to_manifest_entry(&self, home: &Path) -> ManifestEntry {
        ManifestEntry {
            ...
            title: None,
            messaged_at: self.source.stamps_order_on_launch().then(Utc::now),
            closed: false,
        }
    }

    pub fn to_managed_record(
        &self,
...
pub fn launch_spec_for_agent(
    ...
    let launch_command = launch_command.unwrap_or_else(|| build_quick_launch_command(agent_id));
    LaunchSpec {
...
pub fn create_unified(config: &Config, spec: LaunchSpec) -> Result<CreatedWindow> {
    ...
    // Include brand-new sessions in the restore allowlist immediately so a crash
    // before the next daemon poll still brings them back on `sessions up`.
    let _ = crate::session::live_snapshot::remember(config, &spec.sessions_session_id);
...
pub fn create_instant_agent(config: &Config, agent_id: &str) -> Result<CreatedWindow> {
    ...
    crate::session::claim_or_create_quick_agent(
        config,
        &cwd,
        agent_id,
...
pub fn bootstrap_workspaces(config: &Config) -> Result<()> {
    ...
    let mut workspace_tmux_index = HashMap::new();
    for (i, ws) in file.workspace.iter().enumerate() {
        if !Path::new(&ws.cwd).is_dir() {
            crate::session::progress::warn_line(&format!(
                "workspace {} cwd does not exist (skipped): {}",
                ws.title, ws.cwd
            ));
            continue;
        }
        ...
        first_window = false;
        created_any = true;
        workspace_tmux_index.insert(i as u32, created.index);
        if window_name != ws.title {
            let _ = tmux::rename_window(&config.tmux_session, created.index, &ws.title);
        }
    }

    let default_workspace_idx = default_workspace_index(&file);
    let focus = if !created_any {
        seed_agents_session_from_default_workspace(config, &file, default_workspace_idx)?.index
    } else {
        workspace_tmux_index
            .get(&default_workspace_idx)
            .copied()
            .or_else(|| workspace_tmux_index.values().copied().min())
            .context("no workspace windows were bootstrapped")?
    };
...
/// Cold boot when every workspace slot is tombstoned: recreate one workspace so
/// the agents tmux session exists for manifest restore.
///
/// Prefer `default_focus` only when that slot still exists and its cwd is present.
/// If the user has tombstoned the default, removed it from workspaces.toml, or the
/// cwd is missing, fall back to the first remaining entry with a real directory.
/// If no toml slot is usable, seed a console window in `$HOME` so boot can finish.
fn seed_agents_session_from_default_workspace(
    config: &Config,
    file: &BootstrapWorkspacesFile,
    workspace_index: u32,
) -> Result<CreatedWindow> {
    let preferred = if (workspace_index as usize) < file.workspace.len() {
        workspace_index as usize
    } else {
        0
    };

    // Preferred first, then remaining slots in order.
    let mut order: Vec<usize> = Vec::with_capacity(file.workspace.len());
    if !file.workspace.is_empty() {
        order.push(preferred);
        for i in 0..file.workspace.len() {
            if i != preferred {
                order.push(i);
            }
        }
    ...
    for idx in order {
        let ws = &file.workspace[idx];
        if !Path::new(&ws.cwd).is_dir() {
            crate::session::progress::warn_line(&format!(
                "workspace {} cwd does not exist (skipped seed): {}",
                ws.title, ws.cwd
            ));
            continue;
        }
        ...
        // Only reopen the default when we truly have no other way to seed agents.
        // Callers only reach here after every workspace_bootstrap slot was skipped
        // as closed — reopening is intentional, but the slot must still be in toml.
    ...
    // Last resort: no usable workspaces.toml slot — still create agents session.
    let home = config.home.display().to_string();
    let home_cwd = if Path::new(&home).is_dir() {
        home
    } else {
        "/".to_string()
    };
    crate::session::progress::warn_line(&format!(
        "no usable workspace cwd; seeding console in {home_cwd}"
    ));
    let sessions_session_id = bootstrap_sessions_session_id(0, &home_cwd, "");
    let spec = LaunchSpec {
        sessions_session_id,
        source: ManifestSource::WorkspaceBootstrap,
        cwd: home_cwd,
        ...
        workspace_index: None,
        focus: false,
        window_name: Some("session".to_string()),
        bootstrap_new_session: true,
        model_id: None,
        user_prompt: None,
    };
    create_unified(config, spec)
}
...
/// `default_focus` in workspaces.toml is a 1-based workspace slot (1 = first entry).
fn default_workspace_index(file: &BootstrapWorkspacesFile) -> u32 {
    let max = file.workspace.len().saturating_sub(1) as u32;
    file.default_focus.saturating_sub(1).min(max)
}
...
pub fn close_unified(config: &Config, target: CloseTarget) -> Result<CloseOutcome> {
    ...
    // Tombstone the manifest *before* killing the window so a crash / last-window
    // refusal / sessions-up race cannot leave a workspace_bootstrap slot open and
    // have cold-boot restore recreate it (e.g. aeo · copy optimiser zombie).
...
fn resolve_session_for_close(config: &Config, target: &CloseTarget) -> Option<Session> {
    ...
    if let Some(window_index) = target.window_index {
        ...
        if let Some(session) = state.sessions.iter().find(|s| s.tab_index == window_index) {
            return Some(session.clone());
        }
    }
    None
...
mod tests {
    ...
    #[test]
    fn default_workspace_index_is_one_based() {
        let file = BootstrapWorkspacesFile {
            default_focus: 6,
            workspace: vec![
                WorkspaceEntry {
        ...
        assert_eq!(default_workspace_index(&file), 1);
        let file = BootstrapWorkspacesFile {
            default_focus: 1,
            workspace: file.workspace,
        };
        assert_eq!(default_workspace_index(&file), 0);
    }

    #[test]
    fn missing_workspace_cwd_is_detectable_without_bail() {
        // Regression: bootstrap used to `bail!` on the first missing cwd and
        // kill cold boot. Callers must skip + warn instead of failing hard.
        let missing = Path::new("/this/path/definitely/does/not/exist-sessions-test");
    ...
    fn new_chat_manifest_entry_omits_user_prompt_text() {
        ...
        let spec = LaunchSpec {
            ...
            agent: "grok".into(),
            launch_command: crate::agents::build_launch_command_with_prompt(
                "grok",
