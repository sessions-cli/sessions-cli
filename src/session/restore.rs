...
pub fn orchestrate_up(config: &Config) -> Result<()> {
    orchestrate_up_with_options(config, true)
}

/// Like [`orchestrate_up`] but optionally skip tmux attach (for scripts/CI).
pub fn orchestrate_up_with_options(config: &Config, attach: bool) -> Result<()> {
    let progress = Progress::new();
    progress.banner("sessions");

    progress.phase("Loading session list");
    ...
    let allowlist = restore_allowlist(config);
    let pending = entries_needing_restore_with_allowlist(&manifest, &live_set, allowlist.as_ref());
    let cold_boot = !pending.is_empty();
    let agents_missing = !tmux::session_exists(&config.tmux_session);
    let list_done = if cold_boot {
        format!("Session list ready · {} to restore", pending.len())
    } else if agents_missing {
        "Session list ready · cold start".to_string()
    } else {
        "Session list ready".to_string()
    };
    progress.phase_done(&list_done);

    progress.phase("Starting daemon");
    ensure_daemon(config)?;
    send_prepare_restore(config)?;
    progress.phase_done("Daemon ready");

    if cold_boot {
        progress.phase("Opening workspace");
        tmux::bootstrap_session(config)?;
        progress.phase_done("Workspace ready");

        restore_missing_from_manifest_with_progress(config, &manifest, Some(&progress))?;

        if let Some(ref sessions_session_id) = manifest.last_active_sessions_session_id {
            progress.phase("Restoring focus");
            let _ = tmux::select_window_by_sessions_session_id(
                &config.tmux_session,
                sessions_session_id,
            );
            progress.phase_done("Focus restored");
        }
    } else if agents_missing {
        progress.phase("Opening workspace");
        tmux::bootstrap_session(config)?;
    ...
    progress.phase("Reconciling sidebar state");
    send_restore_complete(config)?;
    progress.phase_done("Sidebar state ready");

    progress.phase("Starting sidebar");
    let sessions_bin = sessions_binary();
    tmux::bootstrap_ui_session(&config.tmux_ui_session, &config.tmux_session, &sessions_bin)?;
    tmux::respawn_sidebar_bar(&config.tmux_ui_session, &sessions_bin)?;
    // Drop host-level `tmux attach -t agents` clients; keep the nested workspace attach.
    let _ = tmux::detach_stray_agents_clients(&config.tmux_ui_session, &config.tmux_session);
    tmux::verify_ui_runtime(&config.tmux_ui_session, &config.tmux_session)?;
    progress.phase_done("Sidebar ready");

    if attach {
        progress.final_line("sessions ready — attaching");
        tmux::attach_ui_session(&config.tmux_ui_session)
    } else {
        progress.final_line("sessions ready — attach with: sessions tmux ui attach");
        Ok(())
    }
}

pub fn restore_missing_from_manifest(config: &Config, manifest: &SessionManifest) -> Result<()> {
    restore_missing_from_manifest_with_progress(config, manifest, None)
}

fn restore_missing_from_manifest_with_progress(
    config: &Config,
    ...
    for entry in entries_needing_restore_with_allowlist(manifest, &live_set, allowlist.as_ref()) {
    ...
    for (i, entry) in pending.iter().enumerate() {
        let label = restore_entry_label(entry);
        if let Some(p) = progress {
            p.item(i + 1, total, &label);
        }
        if !Path::new(&entry.cwd).is_dir() {
            let msg = format!(
                "session {} cwd does not exist (skipped): {}",
                entry.sessions_session_id, entry.cwd
            );
            if let Some(p) = progress {
                p.warn(&msg);
            } else {
                crate::session::progress::warn_line(&msg);
            }
            continue;
        }
...
fn restore_entry_label(entry: &ManifestEntry) -> String {
    let title = entry
        .title
        .as_deref()
        .map(str::trim)
        .filter(|t| !t.is_empty());
    let path = entry.cwd_label.trim();
    let agent = entry.agent.trim();
    match title {
        Some(t) if !path.is_empty() => format!("{t} · {path}"),
        Some(t) => t.to_string(),
        None if !path.is_empty() && !agent.is_empty() => format!("{path} · {agent}"),
        None if !path.is_empty() => path.to_string(),
        None if !agent.is_empty() => agent.to_string(),
        None => entry.sessions_session_id.clone(),
    }
}
...
pub(crate) fn needs_cold_boot_restore(
    ...
    needs_cold_boot_restore_with_allowlist(live_sessions_session_ids, manifest, None)
}

pub(crate) fn needs_cold_boot_restore_with_allowlist(
    live_sessions_session_ids: &HashSet<String>,
    manifest: &SessionManifest,
    allowlist: Option<&HashSet<String>>,
) -> bool {
    !entries_needing_restore_with_allowlist(manifest, live_sessions_session_ids, allowlist)
        .is_empty()
}
...
pub(crate) fn entries_needing_restore<'a>(
    ...
    entries_needing_restore_with_allowlist(manifest, live_sessions_session_ids, None)
}

/// Like [`entries_needing_restore`], but when `allowlist` is `Some` only those
/// sessions_session_ids are eligible (last healthy live snapshot).
pub(crate) fn entries_needing_restore_with_allowlist<'a>(
    manifest: &'a SessionManifest,
    live_sessions_session_ids: &HashSet<String>,
    allowlist: Option<&HashSet<String>>,
) -> Vec<&'a ManifestEntry> {
    let mut pending: Vec<&ManifestEntry> = manifest
        ...
        .iter()
        .filter(|entry| {
            if entry.closed || live_sessions_session_ids.contains(&entry.sessions_session_id) {
                return false;
            }
            match allowlist {
                Some(allowed) => allowed.contains(&entry.sessions_session_id),
                None => true,
            }
        })
        .collect();
...
fn restore_allowlist(config: &Config) -> Option<HashSet<String>> {
    crate::session::live_snapshot::load(config)
}
...
mod tests {
    ...
    #[test]
    fn restore_entry_label_prefers_title_and_path() {
        let entry = ManifestEntry {
            sessions_session_id: "ssn_x".into(),
            ...
            title: Some("ship api".into()),
            messaged_at: None,
            closed: false,
        };
        assert_eq!(restore_entry_label(&entry), "ship api · ~/foo");
    }
    ...
    #[test]
    fn entries_needing_restore_respects_live_allowlist() {
        let live_one = sample_dynamic_entry("ssn_live", "/tmp/live", "live");
        let orphan = sample_dynamic_entry("ssn_orphan", "/tmp/orphan", "orphan");
        let manifest = SessionManifest {
            version: 1,
            last_active_sessions_session_id: None,
            migrated_from: None,
            entries: vec![live_one, orphan],
        };
        let allow: HashSet<String> = HashSet::from(["ssn_live".into()]);
        let pending =
            entries_needing_restore_with_allowlist(&manifest, &HashSet::new(), Some(&allow));
        assert_eq!(pending.len(), 1);
        assert_eq!(pending[0].sessions_session_id, "ssn_live");
    }
