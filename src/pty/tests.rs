...
mod facade_migration_tests {
    ...
    use crate::pty::{
        ...
        effective_workspace_command, ensure_app_registry, format_session_title, infer_pane_state,
        is_confident_thread_title, is_machine_derived_thread, is_sticky_thread_title,
        is_weak_thread_name, merge_lifecycle_state, parse_description, poll_foreground_app,
        resolve_agent_app, resolve_session_names, session_names_from_prompt, shorten_command,
        shorten_prompt, PaneKind, CONSOLE_LABEL, DEFAULT_AGENT_APP,
    };
    use crate::session::{
  ...
  "agent_name": "cursor"
    ...
    fn derive_project_from_cwd() {
        assert_eq!(
            derive_project("/home/testuser/projects/acme/acme-frontend", &home()),
            "acme"
        );
        assert_eq!(
    ...
    fn build_description_uses_grok_app() {
        let desc = build_description("fix sidebar", "/home/testuser/projects/acme", &home());
    ...
    fn session_names_from_prompt_uses_grok_app() {
        let names = session_names_from_prompt("Smooth terminal transitions", "acme · frontend dev")
            .unwrap();
    ...
    fn resolve_agent_app_ignores_project_prefix() {
        assert_eq!(resolve_agent_app("acme · frontend dev"), "grok");
  ...
  "agent_name": "cursor"
    ...
    fn idle_grok_workspace_is_not_console() {
        // Unbound idle pane: workspace bootstrap (command=grok + title) must not invent
        // an agent identity — fall back to project/cwd leaf, never CONSOLE.
        let workspace = WorkspaceRef {
            title: "acme · main workspace",
            command: "grok",
        };
        let (title, thread, app) = resolve_session_names(
            &home(),
            "/home/testuser/projects/acme",
            None,
            None,
            "",
            "",
        ...
        assert_eq!(app, "acme");
        assert_eq!(thread, "acme");
        assert_eq!(title, "acme · acme");
    ...
    fn shell_workspace_uses_cwd_leaf_at_project_path() {
        ...
        let (title, thread, app) = resolve_session_names(
            &home(),
            "/home/testuser/projects/acme/acme-backend",
            None,
            None,
            "",
            "",
        ...
        assert!(app.is_empty());
        assert_eq!(thread, "acme-backend");
        assert_eq!(title, "acme-backend");
    }

    #[test]
    fn command_workspace_shows_command_descriptor() {
        let workspace = WorkspaceRef {
            title: "acme · frontend dev",
            command: "./run-local-dev.sh",
        };
        let (title, thread, app) = resolve_session_names(
            &home(),
            "/home/testuser/projects/acme/acme-frontend",
            None,
            None,
            "",
            "",
        ...
        assert_eq!(title, "acme · ./run-local-dev.sh");
    }

    #[test]
    fn agent_workspace_without_session_falls_back_to_project() {
        // Without a bound agent session id / runtime agent, do not apply workspace
        // command agent or catalog thread titles.
        let workspace = WorkspaceRef {
            title: "acme · main workspace",
            command: "grok",
        };
    ...
    #[test]
    fn agent_workspace_with_runtime_agent_uses_workspace_thread() {
        let workspace = WorkspaceRef {
            title: "acme · main workspace",
            command: "grok",
        };
        ...
        assert_eq!(app, "grok");
        // Runtime agent binds command agent, but workspace thread still needs a session id.
        assert_eq!(thread, "?");
    ...
    fn shell_command_beats_stale_workspace_thread_name() {
        ...
        assert!(app.is_empty());
        assert_eq!(thread, "acme-frontend");
        assert_eq!(title, "acme-frontend");
    }
    ...
    #[test]
    fn script_run_identity_prefers_script_over_python() {
        use crate::pty::script_run_identity;
        assert_eq!(
            script_run_identity(
                "python3.13 ./thisisthescript.py",
                "/home/testuser/projects/acme"
            )
            .as_deref(),
            Some("./thisisthescript.py")
        );
        assert_eq!(
            script_run_identity(
                "/usr/bin/env python3 /home/testuser/projects/acme/train.py",
                "/home/testuser/projects/acme"
            )
            .as_deref(),
            Some("./train.py")
        ...
        assert_eq!(
            script_run_identity("python3 -m http.server 8000", "/tmp").as_deref(),
            Some("-m http.server 8000")
        );
        assert_eq!(
            script_run_identity(
            ...
            .as_deref(),
            Some("manage.py runserver 0.0.0.0:8000")
        );
    }

    #[test]
    fn classify_pane_titles_python_script_not_interpreter() {
        match classify_pane(
            "python3.13",
            "python3.13 ./thisisthescript.py",
            "/home/testuser/projects/acme",
            ...
            PaneKind::Tool { app, thread, .. } => {
                assert_eq!(app, "./thisisthescript.py");
                assert_eq!(thread, "./thisisthescript.py");
            }
    ...
    #[test]
    fn resolve_session_names_uses_script_identity_for_python_foreground() {
        ...
        assert_eq!(title, "./thisisthescript.py");
        assert_eq!(thread, "./thisisthescript.py");
    }

    #[test]
    fn poll_foreground_app_keeps_script_labels() {
        assert_eq!(
            poll_foreground_app(false, Some("./thisisthescript.py"), None),
            Some("./thisisthescript.py")
        );
        assert_eq!(
            poll_foreground_app(false, Some("manage.py runserver"), None),
            Some("manage.py runserver")
        );
    }

    #[test]
    fn idle_unbound_shell_uses_cwd_leaf_not_console() {
        // Idle unbound poll has no workspace command — must not force "console"
        // for project directories (regression from script-title work).
        let (title, thread, _) = resolve_session_names(
            &home(),
            "/home/testuser/projects/acme/acme-frontend",
            None,
    ...
    #[test]
    fn project_dir_console_window_name_does_not_stick() {
        let (title, thread, _) = resolve_session_names(
            &home(),
            "/home/testuser/projects/acme",
            None,
    ...
    #[test]
    fn home_console_still_labels_console() {
        let (title, thread, _) = resolve_session_names(
            &home(),
            "/home/testuser",
            None,
        ...
        assert_eq!(thread, CONSOLE_LABEL);
    }

    #[test]
    fn console_is_not_a_sticky_thread_title() {
        assert!(!is_sticky_thread_title("console"));
        assert!(!is_sticky_thread_title("raw terminal"));
        assert!(is_sticky_thread_title("frontend dev"));
    }
    ...
    #[test]
    fn shorten_prompt_rejects_script_and_mcp_paths() {
        // Poll once fed classify identity as prompt; stripping / and . produced
        // "localbingoogle-calendar-mcp" as a fake thread title.
        assert_eq!(shorten_prompt("./.local/bin/google-calendar-mcp"), "");
        assert_eq!(
            shorten_prompt("/home/testuser/.local/bin/google-calendar-mcp"),
            ""
        );
        assert_eq!(shorten_prompt("manage.py"), "");
        assert_eq!(shorten_prompt("./train.py"), "");
        // Real multi-word prompts still work.
        assert_eq!(
            shorten_prompt("fix the sidebar session titles"),
            "fix the sidebar session titles"
        );
    }

    #[test]
    fn resolve_session_names_does_not_mangle_mcp_script_path_via_prompt() {
        // Even if a path leaks into the prompt slot, it must not become a title.
        let (title, thread, _) = resolve_session_names(
            &home(),
            "/home/testuser",
            Some("./.local/bin/google-calendar-mcp"),
            None,
            "console",
            "./.local/bin/google-calendar-mcp",
            "./.local/bin/google-calendar-mcp",
            None,
            false,
        );
        assert!(
            !thread.contains("localbingoogle"),
            "mangled MCP path leaked into thread: {thread}"
        );
        assert!(
            !title.contains("localbingoogle"),
            "mangled MCP path leaked into title: {title}"
        );
        // Live script identity should stay as the path (or bare script), not a slug.
        assert!(
            thread.contains("google-calendar-mcp") || thread == "./.local/bin/google-calendar-mcp",
            "expected script identity, got {thread}"
        );
    }

    #[test]
    fn resolve_session_names_labels_live_grok_over_console_at_home() {
        // Home console pane that is actually running grok (manual launch).
        let (title, thread, app) = resolve_session_names(
            &home(),
            "/home/testuser",
            Some("grok"),
        ...
        assert_eq!(app, "grok");
        assert!(title.starts_with("grok"), "title={title}");
        assert_ne!(thread, "localbingoogle-calendar-mcp");
        assert!(!title.contains("google-calendar"));
    }
    ...
    fn workspace_ref_requires_matching_window_index_not_just_cwd() {
        let catalog = WorkspaceCatalog {
            entries: vec![WorkspaceEntry {
                title: "acme · frontend dev".into(),
                cwd: "/home/testuser/projects/acme/acme-frontend".into(),
                command: "./run-local-dev.sh".into(),
            }],
        };
        assert!(catalog
            .workspace_ref_for_window(0, "/home/testuser/projects/acme/acme-frontend")
            .is_some());
        assert!(catalog
            .workspace_ref_for_window(19, "/home/testuser/projects/acme/acme-frontend")
            .is_none());
    }
    ...
    fn workspace_ref_matches_index_and_cwd_together() {
        let catalog = WorkspaceCatalog {
            entries: vec![
                WorkspaceEntry {
                    title: "acme · dashboard agent".into(),
                    cwd: "/home/testuser/projects/acme/acme-dashboard".into(),
                    command: "grok".into(),
                },
                WorkspaceEntry {
    ...
    fn workspace_ref_ignores_index_when_cwd_does_not_match() {
        let catalog = WorkspaceCatalog {
            entries: vec![WorkspaceEntry {
                title: "acme · dashboard local".into(),
                cwd: "/home/testuser/projects/acme/acme-dashboard".into(),
