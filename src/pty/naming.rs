...
fn is_bootstrap_command_label(thread: &str) -> bool {
    let thread = thread.trim();
    thread.starts_with("./")
        || thread.starts_with("../")
        || (thread.contains('/')
            ...
            && !thread.contains(' '))
        || (thread.ends_with(".sh") && !thread.contains(' '))
}
...
pub fn shorten_prompt(raw: &str) -> String {
    ...
    let re_cmd = Regex::new(r"(?i)^/(implement|review|design|check-work|best-of-n)\s+").unwrap();
    let text = re_cmd.replace(text, "").to_string();
    let re_ws = Regex::new(r"\s+").unwrap();
    let text = re_ws.replace_all(&text, " ").to_string();
    let re_chars = Regex::new(r"[^\w\s·\-]").unwrap();
    let text = re_chars.replace_all(&text, "").to_string();
...
pub fn poll_foreground_app<'a>(
    ...
    classify_app
        .filter(|app| is_agent_app(app) || !is_machine_derived_thread(app))
        .or(runtime_agent)
}
...
pub fn is_foreground_tool_identity(title: &str, description: &str) -> bool {
    ...
    if description.is_empty()
        || is_weak_thread_name(description)
        || is_console_label(description)
        || is_machine_derived_thread(description)
        || is_agent_app(description)
    {
        return false;
    }
...
pub fn resolve_session_names(
    ...
    let summary = agent_session_id
        .and_then(|id| crate::agents::load_session_summary(home, &lookup_cwd, id));
    ...
    let command_agent = agent_from_command(workspace_command);
    let existing_agent = parse_app(existing_title).filter(|app| is_agent_app(app));
    let has_existing_agent = existing_agent.is_some();
    let is_shell_pane = !workspace_command.trim().is_empty() && is_shell_command(workspace_command);
    ...
    if let Some(tool_app) = foreground_app.filter(|app| !is_agent_app(app)) {
        let classified = non_weak_thread(existing_description).filter(|thread| {
