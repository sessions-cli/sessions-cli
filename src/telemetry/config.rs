...
/// Max quick-launch badges on a PWD group header.
pub const GROUP_LAUNCH_MAX: usize = 4;

/// Default PWD group badges: Grok, OpenCode, console.
pub fn default_group_launch() -> Vec<String> {
    vec!["grok".into(), "opencode".into(), "console".into()]
}

/// Cycle order when editing a quick-launch slot in settings (includes Off).
pub const GROUP_LAUNCH_CYCLE: &[&str] = &["", "grok", "codex", "claude", "opencode", "console"];
...
pub struct SidebarSection {
    ...
    /// Agent ids for PWD group hover badges (max [`GROUP_LAUNCH_MAX`]).
    /// Valid ids: grok, codex, claude, opencode, console.
    #[serde(default = "default_group_launch")]
    pub group_launch: Vec<String>,
    /// When true (default), new-session panel opens with agent + model already
    /// confirmed from saved defaults and focus on the directory/path field.
    /// When false, focus starts on Agent (previous behavior).
    /// `config.toml` `[sidebar] new_session_preselect_agent_model = false`.
    #[serde(default = "default_true")]
    pub new_session_preselect_agent_model: bool,
}

impl Default for SidebarSection {
    fn default() -> Self {
        Self {
            new_session_rank: WorkspaceRankMode::default(),
            group_launch: default_group_launch(),
            new_session_preselect_agent_model: true,
        }
    }
}

/// Normalize configured launch agents: lowercase, known ids only, cap at max.
pub fn normalize_group_launch(raw: &[String]) -> Vec<String> {
    let mut out = Vec::new();
    for id in raw {
        ...
        if !GROUP_LAUNCH_CYCLE.iter().any(|c| *c == id) {
            continue;
        }
        out.push(id);
        if out.len() >= GROUP_LAUNCH_MAX {
            break;
        }
    }
    out
...
/// Next value when cycling a settings slot (`current` may be empty for Off).
pub fn cycle_group_launch_agent(current: &str) -> &'static str {
    let cur = current.trim().to_ascii_lowercase();
    let idx = GROUP_LAUNCH_CYCLE
        .iter()
        .position(|id| *id == cur.as_str())
        .unwrap_or(0);
    GROUP_LAUNCH_CYCLE[(idx + 1) % GROUP_LAUNCH_CYCLE.len()]
}

pub fn load_group_launch(home: &Path) -> Vec<String> {
    SessionsConfig::load(home)
        .map(|cfg| normalize_group_launch(&cfg.sidebar.group_launch))
        .unwrap_or_else(|_| default_group_launch())
}

pub fn save_group_launch(home: &Path, agents: &[String]) -> Result<()> {
    let mut cfg = SessionsConfig::load(home).unwrap_or_default();
    cfg.sidebar.group_launch = normalize_group_launch(agents);
    cfg.save(home)
}

/// Whether new-session auto-confirms agent/model and focuses the path field (default: on).
pub fn load_new_session_preselect_agent_model(home: &Path) -> bool {
    SessionsConfig::load(home)
        .map(|cfg| cfg.sidebar.new_session_preselect_agent_model)
        .unwrap_or(true)
}

pub fn save_new_session_preselect_agent_model(home: &Path, enabled: bool) -> Result<()> {
    let mut cfg = SessionsConfig::load(home).unwrap_or_default();
    cfg.sidebar.new_session_preselect_agent_model = enabled;
    cfg.save(home)
}

/// Toggle new-session agent/model preselect. Returns the new value.
pub fn toggle_new_session_preselect_agent_model(home: &Path) -> Result<bool> {
    let mut cfg = SessionsConfig::load(home).unwrap_or_default();
    cfg.sidebar.new_session_preselect_agent_model = !cfg.sidebar.new_session_preselect_agent_model;
    let next = cfg.sidebar.new_session_preselect_agent_model;
    cfg.save(home)?;
    Ok(next)
}

/// Display label for a launch agent id (or "Off").
pub fn group_launch_label(agent_id: &str) -> &'static str {
...
/// One-letter badge glyph for a launch agent (`[G]`, `[+]`, …).
pub fn group_launch_badge(agent_id: &str) -> &'static str {
    match agent_id.trim().to_ascii_lowercase().as_str() {
        "grok" => "[G]",
        "codex" => "[X]",
        "claude" => "[C]",
...
impl UpdateInfo {
    ...
    fn from_cache_with_options(cache: &UpdateCache, ignore_dismiss: bool) -> Option<Self> {
        ...
        let dismissed =
            (!cache.dismissed_version.is_empty()).then(|| cache.dismissed_version.clone());
        if !ignore_dismiss
