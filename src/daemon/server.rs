...
pub async fn run_daemon(config: Config, foreground: bool) -> Result<()> {
    ...
    let config_automation = config.clone();
    tokio::spawn(async move {
        crate::daemon::automation::automation_loop(config_automation).await;
    });
...
async fn try_auto_reconcile(state: Arc<DaemonState>, config: Config, event_tx: Arc<EventSender>) {
    ...
    let live_set: HashSet<String> =
        crate::daemon::tmux::list_live_sessions_session_ids(&config.tmux_session)
            ...
            .into_keys()
            .collect();
    let allowlist = crate::session::live_snapshot::load(&config);
    if crate::session::needs_cold_boot_restore_with_allowlist(
        &live_set,
        &manifest,
        allowlist.as_ref(),
    ) {
...
async fn poll_loop(state: Arc<DaemonState>, config: Config, event_tx: Arc<EventSender>) {
    ...
    loop {
        ...
        // Top up pre-hydrated grok/opencode spares every few polls for live cwds.
        warm_tick = warm_tick.wrapping_add(1);
        if warm_tick % 4 == 0 && !state.is_booting().await {
            let cfg = config.clone();
            let cwds = state.live_cwd_labels().await;
            tokio::task::spawn_blocking(move || {
                crate::session::maintain_warm_pool(&cfg, &cwds);
            });
        }
...
async fn handle_command(
    ...
    match cmd {
        ...
        ClientCommand::Focus {
            ...
            // Latency path matching sidebar click: switch the tmux window first,
            // ack the CLI immediately, then commit is_active + broadcast snapshot
            // for the sidebar (snapshot serialize must not delay the switch).
            match state.resolve_focus_window(window_index, tab_index).await {
                Ok(target) => {
                    if let Err(e) =
                        crate::daemon::tmux::select_window(&state.config().tmux_session, target)
                    {
                        warn!("focus select_window failed: {e}");
                        ...
                        writer
                            .write_all((ack.to_string() + "\n").as_bytes())
                            .await?;
                        let event = state.commit_focus_state(target).await;
                        let _ = event_tx.send(event);
                    }
                }
                Err(e) => {
