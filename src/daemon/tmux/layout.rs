...
/// Share one geometry probe across adjacent `current_pane_*` callers in a tick.
const GEOMETRY_CACHE_TTL: Duration = Duration::from_millis(50);
...
static GEOMETRY_CACHE: LazyLock<Mutex<Option<GeometryCacheEntry>>> =
    LazyLock::new(|| Mutex::new(None));

/// Sessions chrome base — terminal OSC 11 backdrop and tmux window bg.
const SESSIONS_BACKDROP_BG: &str = "#000000";
/// Host terminal tab/window title — must not show `tmux attach-session …`.
/// Note: Cursor/VS Code `${process}` still reports the real binary (`tmux`)
/// because `exec -a` only changes argv0 (macOS `ucomm` stays tmux). Cursor
/// tab labels are forced to this string in `bin/setup-cursor.sh`.
...
const SIDEBAR_BOOTSTRAP_WIDTH: u16 = 54;
pub const WORKSPACE_MIN_WIDTH: u16 = 48;
/// Temporary workspace floor while the sidebar is force-peeked on a narrow client.
pub const PEEK_WORKSPACE_MIN: u16 = 24;
/// Matches `ui::COLLAPSED_PANE_WIDTH` — micro status rail for narrow clients.
pub const COLLAPSED_SIDEBAR_WIDTH: u16 = 4;
/// Matches `ui::DRAG_COLLAPSE_AT_OR_BELOW` — allow dragging into the snap zone.
const DRAG_COLLAPSE_AT_OR_BELOW: u16 = 16;
const MIN_EXPANDED_SIDEBAR_WIDTH: u16 = 22;
...
/// `(client_width, pane_width)` for the current pane in one tmux round-trip.
///
/// The responsive-width path needs both fields every tick; spawning twice
/// doubled idle sidebar cost. Result is memoized briefly so adjacent callers
/// in the same event-loop iteration share one probe.
pub fn current_pane_geometry() -> Option<(u16, u16)> {
    ...
    {
        let cache = GEOMETRY_CACHE.lock().ok()?;
        if let Some(hit) = cache.as_ref() {
            if hit.pane == pane && hit.at.elapsed() < GEOMETRY_CACHE_TTL {
                return Some((hit.client_width, hit.pane_width));
            }
        }
    }
    let output = run_tmux(&[
    ...
    let body = body.trim();
    let (client_raw, pane_raw) = body.split_once('\t')?;
    let pane_width: u16 = pane_raw.trim().parse().ok()?;
    let client_width = match client_raw.trim().parse::<u16>() {
        Ok(w) if w > 0 => w,
        // Fallback when client_width is empty/zero in this context.
        _ => client_width_for_target(&pane),
    };
    if let Ok(mut cache) = GEOMETRY_CACHE.lock() {
        *cache = Some(GeometryCacheEntry {
    ...
    Some((client_width, pane_width))
}

pub fn current_pane_width() -> Option<u16> {
    if let Some((_, pane_width)) = current_pane_geometry() {
        return Some(pane_width);
    }
    if std::env::var("TMUX").is_err() {
        return None;
    }
...
pub fn ui_window_active_pane_index(ui_session: &str) -> Option<u32> {
    let (workspace_active, sidebar_active) = ui_window_focus_snapshot(ui_session)?;
    if workspace_active {
        Some(1)
...
/// Single tmux round-trip for UI window focus: (workspace pane active, sidebar pane active).
pub fn ui_window_focus_snapshot(ui_session: &str) -> Option<(bool, bool)> {
    ...
    let mut workspace_active = false;
    let mut sidebar_active = false;
    for line in String::from_utf8_lossy(&output.stdout).lines() {
        let mut parts = line.split_whitespace();
        let index: u32 = parts.next()?.parse().ok()?;
        let active = parts.next()? == "1";
        match index {
            0 if active => sidebar_active = true,
            1 if active => workspace_active = true,
            _ => {}
        }
    }
    Some((workspace_active, sidebar_active))
}
...
pub fn clamp_sidebar_width(desired: u16, client_width: u16) -> u16 {
    clamp_sidebar_width_with_workspace_min(desired, client_width, WORKSPACE_MIN_WIDTH)
}

/// Like [`clamp_sidebar_width`] but with an explicit workspace floor (peek expand uses a lower one).
pub fn clamp_sidebar_width_with_workspace_min(
    desired: u16,
    client_width: u16,
    workspace_min: u16,
) -> u16 {
    // Allow the micro rail through without bumping to expanded min.
    if desired > 0 && desired <= COLLAPSED_SIDEBAR_WIDTH {
        return desired;
    }
    let max_allowed = client_width.saturating_sub(workspace_min);
    // Allow dragging into the collapse snap zone (below a usable list width).
    // The bar app snaps to the rail when width <= DRAG_COLLAPSE_AT_OR_BELOW.
    let min_expanded = if workspace_min < WORKSPACE_MIN_WIDTH {
        // Peek: keep a compact but readable list.
        28u16.min(max_allowed.max(1))
    } else {
        DRAG_COLLAPSE_AT_OR_BELOW.min(MIN_EXPANDED_SIDEBAR_WIDTH)
    };
    desired.clamp(min_expanded.max(1), max_allowed.max(min_expanded.max(1)))
}

pub fn current_pane_client_width() -> Option<u16> {
    if let Some((client_width, _)) = current_pane_geometry() {
        return Some(client_width);
    }
    if std::env::var("TMUX").is_err() {
        return None;
    }
...
pub fn resize_pane_width_at(target: &str, desired: u16) -> Result<()> {
    resize_pane_width_at_with_workspace_min(target, desired, WORKSPACE_MIN_WIDTH)
}

pub fn resize_pane_width_at_with_workspace_min(
    target: &str,
    desired: u16,
    workspace_min: u16,
) -> Result<()> {
    let client_width = client_width_for_target(target);
    let width = clamp_sidebar_width_with_workspace_min(desired, client_width, workspace_min);
...
pub fn resize_current_pane_width(desired: u16) -> Result<()> {
    resize_current_pane_width_with_workspace_min(desired, WORKSPACE_MIN_WIDTH)
}

pub fn resize_current_pane_width_with_workspace_min(
    desired: u16,
    workspace_min: u16,
) -> Result<()> {
    if std::env::var("TMUX").is_err() {
    ...
    resize_pane_width_at_with_workspace_min(&pane, desired, workspace_min)
}
...
/// Sidebar pane target in the UI session (`sessions-ui:ui.0`).
pub fn ui_sidebar_pane_target(ui_session: &str) -> String {
    format!("{ui_session}:ui.0")
}

/// Grow/shrink the sessions-ui sidebar pane by `delta` columns (positive = wider).
///
/// Used by `sessions resize-sidebar` (Cursor keybinds / tmux Meta) when the bar
/// process does not have keyboard focus. The bar adopts the new width on its
/// next responsive sync when client width is stable.
pub fn resize_ui_sidebar_by(ui_session: &str, delta: i16) -> Result<()> {
    if delta == 0 {
        return Ok(());
    }
    let target = ui_sidebar_pane_target(ui_session);
    if !session_exists(ui_session) {
        anyhow::bail!("tmux session {ui_session} missing; run sessions up first");
    }
    let client_width = client_width_for_target(&target);
    let current = pane_width_at(&target).unwrap_or(SIDEBAR_BOOTSTRAP_WIDTH);

    // Grow from micro rail → open to default list width.
    if current <= COLLAPSED_SIDEBAR_WIDTH {
        if delta < 0 {
            return Ok(());
        }
        let open_to = clamp_sidebar_width(SIDEBAR_BOOTSTRAP_WIDTH, client_width);
        return resize_pane_width_at_fast(&target, open_to);
    }
    ...
    // Mirror bar drag-collapse: at/below snap zone → micro rail.
    if desired <= DRAG_COLLAPSE_AT_OR_BELOW {
        return resize_pane_width_at_fast(&target, COLLAPSED_SIDEBAR_WIDTH);
    }

    let width = clamp_sidebar_width(desired.max(MIN_EXPANDED_SIDEBAR_WIDTH), client_width);
    resize_pane_width_at_fast(&target, width)
}
...
///
/// **Grok / OpenCode native select/copy:**
/// - **Drag:** always `send-keys -M` for the whole agents session (not only when
///   `mouse_any_flag` is set). If we fall through to `copy-mode -M` whenever
///   mouse tracking flickers off, tmux steals the cursor and Grok only “sometimes”
///   gets selection.
/// - **Copy:** Grok emits OSC 52; nested attach does not re-emit that to the host
///   clipboard. `pane-set-clipboard` → `pbcopy` (see
///   `install_osc52_os_clipboard_hook`) bridges paste-buffer → OS clipboard.
///
...
/// Reinstall root mouse bindings (wheel/click/drag/up). Safe to call on reload.
pub fn refresh_ui_mouse_bindings(ui_session: &str, agents_session: &str) -> Result<()> {
...
fn configure_ui_mouse_focus(ui_session: &str, agents_session: &str) -> Result<()> {
    ...
    // or accidental drag→copy-mode that leaves a tmux cursor in the workspace.
    let _ = run_tmux(&["copy-mode", "-q", "-t", ui_session]);
    exit_all_copy_mode(agents_session);
    ...
    run_tmux(&["bind-key", "-T", "root", "WheelUpPane", &wheel_up])
        .with_context(|| format!("install WheelUpPane binding for {ui_session}"))?;
    run_tmux(&["bind-key", "-T", "root", "WheelDownPane", &wheel_down])
        .with_context(|| format!("install WheelDownPane binding for {ui_session}"))?;
    run_tmux(&["bind-key", "-T", "root", "MouseDrag1Pane", &drag])
        .with_context(|| format!("install MouseDrag1Pane binding for {ui_session}"))?;
    run_tmux(&["bind-key", "-T", "root", "MouseUp1Pane", &up])
        .with_context(|| format!("install MouseUp1Pane binding for {ui_session}"))?;
    run_tmux(&["bind-key", "-T", "root", "MouseDown1Pane", &click])
        .with_context(|| format!("install MouseDown1Pane binding for {ui_session}"))?;
    // Override tmux defaults — DoubleClick/TripleClick copy-mode chains flash syntax
    // errors in the UI session and can take multiple clicks to reach the workspace.
    run_tmux(&["bind-key", "-T", "root", "DoubleClick1Pane", &click])
        .with_context(|| format!("install DoubleClick1Pane binding for {ui_session}"))?;
    run_tmux(&["bind-key", "-T", "root", "TripleClick1Pane", &click])
        .with_context(|| format!("install TripleClick1Pane binding for {ui_session}"))?;
    ...
    .context("install MouseDrag1Border binding")?;
    verify_ui_mouse_bindings(ui_session)
}

/// Confirm server-global mouse bindings were installed for the UI session.
pub fn verify_ui_mouse_bindings(ui_session: &str) -> Result<()> {
    let click = run_tmux(&["list-keys", "-T", "root", "MouseDown1Pane"])?;
    let click_binding = String::from_utf8_lossy(&click.stdout);
    if !click_binding.contains(ui_session) {
        anyhow::bail!(
            "MouseDown1Pane binding missing {ui_session} guard (got: {})",
            click_binding.trim()
        );
    }
    for key in ["DoubleClick1Pane", "TripleClick1Pane"] {
        let output = run_tmux(&["list-keys", "-T", "root", key])?;
        let binding = String::from_utf8_lossy(&output.stdout);
        if !binding.contains(ui_session) {
            anyhow::bail!(
                "{key} binding missing {ui_session} guard (got: {})",
                binding.trim()
            );
        }
        if binding.contains("copy-mode") {
            anyhow::bail!(
                "{key} must not enter copy-mode in the UI session (got: {})",
                binding.trim()
            );
        }
    }
...
/// Kill and respawn the sidebar bar process so install/reload/up pick up the new binary
/// and reset ratatui mouse capture state.
pub fn respawn_sidebar_bar(ui_session: &str, sessions_bin: &Path) -> Result<()> {
    if !session_exists(ui_session) {
        anyhow::bail!("tmux session {ui_session} missing; run sessions tmux ui bootstrap first");
    }
    let target = ui_sidebar_target(ui_session);
    ...
    .with_context(|| format!("respawn sidebar bar at {target}"))?;
    Ok(())
}

/// Runtime checks after bootstrap/reload/up — fail loud instead of leaving a half-initialized UI.
pub fn verify_ui_runtime(ui_session: &str, agents_session: &str) -> Result<()> {
    if !session_exists(ui_session) {
        anyhow::bail!("tmux session {ui_session} missing");
    }
    let mouse = run_tmux(&["show-options", "-t", ui_session, "mouse"])?;
    let mouse_line = String::from_utf8_lossy(&mouse.stdout);
    if !mouse_line.contains("mouse on") {
        anyhow::bail!("{ui_session} mouse option is not on ({mouse_line})");
    }
    if session_exists(agents_session) {
        let agents_mouse = run_tmux(&["show-options", "-t", agents_session, "mouse"])?;
        let agents_line = String::from_utf8_lossy(&agents_mouse.stdout);
        if !agents_line.contains("mouse on") {
            anyhow::bail!("{agents_session} mouse option is not on ({agents_line})");
        }
    }
    verify_ui_mouse_bindings(ui_session)?;
    let panes = list_ui_panes(ui_session)?;
    if panes.len() != 2 {
        anyhow::bail!("expected two panes in {ui_session}, found {}", panes.len());
    }
    if !panes
        .iter()
        .any(|pane| pane.index == 0 && is_sidebar_start_command(&pane.start_command))
    {
        anyhow::bail!("{ui_session} pane 0 is not running sessions bar");
    }
    Ok(())
}

/// Forward mouse to the pane under the cursor. UI-session clicks select the pane
/// under the mouse so one click focuses sidebar or workspace for typing; panel
/// dismiss is handled in the bar app.
/// Select the workspace pane in the UI window (restore keyboard focus after hover probes).
pub fn select_ui_workspace_pane(ui_session: &str) -> Result<()> {
    let target = format!("{ui_session}:ui.1");
    run_tmux(&["select-pane", "-t", &target])
        .with_context(|| format!("select workspace pane at {target}"))?;
    Ok(())
}

pub(crate) fn ui_click_binding(ui_session: &str) -> String {
    let in_ui = format!("#{{==:#{{session_name}},{ui_session}}}");
    // Single-quoted command strings with a literal `;`. Brace groups inside
    // if-shell branches make tmux report a syntax error when the branch runs;
    // `\;` is treated literally inside quotes.
    // send-keys -M without -t =: nested agents client re-dispatches the injected
    // mouse event; = is invalid there and flashes a yellow syntax error bar.
    format!("if-shell -F '{in_ui}' 'select-pane -t = ; send-keys -M' 'send-keys -M'")
}
...
/// Combined guard: true for the UI session or for the agents session when the
/// pane owns the mouse (a TUI or copy mode). Used for wheel/drag/up so a
/// single flat if-shell can replace nested braces, which tmux rejects at runtime.
///
/// When `mouse_any_flag` is set (Grok, OpenCode, …), drag is forwarded so the
/// TUI can run **native** select/copy — do not drop that branch for copy-mode.
fn ui_or_agents_mouse_guard(ui_session: &str, agents_session: &str) -> String {
    format!(
        "#{{||:#{{==:#{{session_name}},{ui_session}}},#{{&&:#{{==:#{{session_name}},{agents_session}}},#{{||:#{{pane_in_mode}},#{{mouse_any_flag}}}}}}}}"
    )
}

/// Exit copy-mode on every pane in `session` (best-effort). Used on mouse rebind
/// so a mistaken copy-mode entry does not leave a sticky tmux cursor.
fn exit_all_copy_mode(session: &str) {
    if !session_exists(session) {
        return;
    }
    let Ok(out) = run_tmux(&["list-panes", "-t", session, "-F", "#{pane_id}"]) else {
        return;
    };
    for pane in String::from_utf8_lossy(&out.stdout).lines() {
        let pane = pane.trim();
        if !pane.is_empty() {
            let _ = run_tmux(&["copy-mode", "-q", "-t", pane]);
        }
    }
}
...
///
/// Agents session **always** forwards drag (Grok/OpenCode native select). Never
/// gate on `mouse_any_flag` — that flag flickers and copy-mode steals the gesture.
pub(crate) fn ui_drag_binding(ui_session: &str, agents_session: &str) -> String {
    let guard = ui_or_agents_forward_guard(ui_session, agents_session);
    format!("if-shell -F '{guard}' 'send-keys -M' 'copy-mode -M'")
}

/// Root mouse-up binding — same pane-stickiness as drag so divider resize can end.
/// Agents always forward so Grok selection can finish on mouse-up.
pub(crate) fn ui_up_binding(ui_session: &str, agents_session: &str) -> String {
    let guard = ui_or_agents_forward_guard(ui_session, agents_session);
    format!("if-shell -F '{guard}' 'send-keys -M'")
}

/// Forward mouse for UI chrome and every agents pane (no mouse_any gate).
fn ui_or_agents_forward_guard(ui_session: &str, agents_session: &str) -> String {
    format!(
        "#{{||:#{{==:#{{session_name}},{ui_session}}},#{{==:#{{session_name}},{agents_session}}}}}"
    )
}
...
pub(crate) fn ui_wheel_binding(
    ...
    let guard = ui_or_agents_mouse_guard(ui_session, agents_session);
    // UI wheel: forward only — do not select-pane on scroll (steals workspace focus
    // and can flash syntax errors when the trackpad fires wheel during hover/clicks).
    match direction {
        WheelDirection::Up => {
            format!("if-shell -F '{guard}' 'send-keys -M' 'copy-mode -eH ; send-keys -M'")
        }
        // At the bottom with no mode active there is nothing to scroll down into.
        WheelDirection::Down => format!("if-shell -F '{guard}' 'send-keys -M'"),
    }
}
...
fn configure_ui_session(ui_session: &str, agents_session: &str) -> Result<()> {
    ...
    // Re-assert on every UI layout configure (make reload / ensure) so existing
    // machines pick this up without a full `sessions up` re-bootstrap.
    disable_auto_window_switch(agents_session);
...
pub fn configure_workspace_session(session: &str) -> Result<()> {
    ...
    disable_auto_window_switch(session);
...
/// Options that prevent tmux from auto-selecting a workspace window.
///
/// Global defaults often include `bell-action any` / `activity-action other`.
/// When a background agent finishes and rings BEL (common for CLI agents),
/// that would `select-window` into the right pane and yank the user away from
/// the session they were reading. Right-pane switching must only happen from
/// explicit focus (sidebar click, `sessions focus`, ⌘N) — never from finish
/// signals.
pub(crate) fn auto_window_switch_disable_options() -> &'static [(&'static str, &'static str)] {
    &[("bell-action", "none"), ("activity-action", "none")]
}

fn disable_auto_window_switch(session: &str) {
    for (opt, val) in auto_window_switch_disable_options() {
        if let Err(e) = run_tmux(&["set-option", "-t", session, opt, val]) {
            debug!("tmux set-option {opt}={val} for {session}: {e}");
        }
    }
}
...
fn ui_sidebar_bar_command(sessions_bin: &Path) -> String {
    // Drop NO_COLOR and friends inherited from monochrome agent shells so the
    // sidebar does not start greyscale (see `color_env`).
    // `exec` replaces the shell; `env` must immediately precede the sessions binary.
    format!(
        "exec {} {} bar",
        crate::color_env::shell_env_prefix(),
        shell_quote(&sessions_bin.display().to_string())
    )
}
...
pub fn attach_ui_session(ui_session: &str) -> Result<()> {
    ...
    // Tag UI session from this process env (Cursor/Ghostty). The bar only sees TERM_PROGRAM=tmux.
    crate::bar::host_terminal::mark_ui_session_host(ui_session);
...
pub fn host_terminal_title_sequence() -> String {
    // BEL + ST forms — xterm.js (Cursor/VS Code) accepts either; some builds
    // only apply one. OSC 0/1/2 cover icon name + window/tab title.
    format!(
        "\x1b]0;{t}\x07\x1b]1;{t}\x07\x1b]2;{t}\x07\
\x1b]0;{t}\x1b\\\x1b]1;{t}\x1b\\\x1b]2;{t}\x1b\\",
        t = HOST_TERMINAL_TITLE
    )
}

pub fn host_terminal_title_bash_printf() -> String {
    // Keep in sync with host_terminal_title_sequence (BEL + ST, OSC 0/1/2).
    format!(
        "printf '\\033]0;{0}\\007\\033]1;{0}\\007\\033]2;{0}\\007\
\\033]0;{0}\\033\\\\\\033]1;{0}\\033\\\\\\033]2;{0}\\033\\\\'",
        HOST_TERMINAL_TITLE
    )
}
...
pub fn host_terminal_backdrop_sequence() -> String {
    // OSC 11 is supported by xterm.js (VS Code / Cursor terminal). Emit BEL + ST
    // like titles — some builds only apply one terminator. Color is `#RRGGBB`
    // (xterm.js-supported XParseColor form). SGR 49 resets cell bg to default
    // so the OSC 11 fill shows through padding.
    format!(
        "{}\x1b]11;{bg}\x07\x1b]11;{bg}\x1b\\\x1b[49m",
        host_terminal_title_sequence(),
        bg = SESSIONS_BACKDROP_BG,
    )
}
...
/// Also re-push the host tab title so Cursor/VS Code do not fall back to "tmux".
pub(crate) fn terminal_backdrop_shell() -> String {
    format!(
        "{} && printf '\\033]11;{SESSIONS_BACKDROP_BG}\\033\\\\\\033[49m\\033[40m\\033[2J\\033[H'",
        host_terminal_title_bash_printf()
    )
}
...
pub(crate) fn console_shell_command() -> String {
    // Force color env so login shells/agents are not monochrome when tmux was
    // started from an agent/CI shell that exported NO_COLOR=1.
    format!(
        "{}; exec {} /bin/zsh -l",
...
pub(crate) fn workspace_shell_command(command: &str) -> String {
    ...
    // Managed launches already clear NO_COLOR via wrap_managed_launch_command;
    // only the fallback login shell needs an env prefix here.
    format!(
