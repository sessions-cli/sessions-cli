...
use crate::daemon::state::identity::{
    ...
    finalize_session_identity, hydrate_agent_binding_from_managed_record, pane_index_from_sessions,
    refresh_merge_prior, repair_session_managed_identity, resolve_agent_session_id,
};
...
pub(crate) fn preserve_hook_cwd_over_stale_poll(
    config: &Config,
    existing: &Session,
    fresh: &mut Session,
) {
...
pub(crate) fn acknowledge_completion_on_focus(
...
pub(crate) struct RefreshComputation {
    ...
    /// Managed sessions_session_ids seen this poll (raw, before closed filter).
    pub(crate) polled_ssns: std::collections::HashSet<String>,
    ...
    /// Closed SSNs that are still live in tmux — tombstones to reopen.
    pub(crate) healed_closed_ssns: Vec<String>,
}
...
pub(crate) fn compute_refresh(
    ...
    let polled_sids: std::collections::HashSet<String> = polled
        ...
        .collect();
    let polled_ssns: std::collections::HashSet<String> = polled
        .iter()
        .filter_map(|s| s.sessions_session_id.clone())
        .collect();

    // A successful close removes the window. If a closed SSN is still on a live
    // pane, the tombstone is stale (false vanish after index churn, or kill
    // failed) — heal it so the row returns to the sidebar.
    let mut healed_closed_ssns = Vec::new();
    let mut healed_set = std::collections::HashSet::new();
    let closed: std::collections::HashSet<ClosedSessionMarker> = closed
        .iter()
        .filter(|marker| {
            if let Some(ssn) = marker.sessions_session_id.as_ref() {
                if polled_ssns.contains(ssn) {
                    if healed_set.insert(ssn.clone()) {
                        healed_closed_ssns.push(ssn.clone());
                    }
                    return false;
                }
            }
    ...
    for mut fresh in polled {
        ...
        // Repair cross-agent identity drift against the durable managed record
        // before merging prior state or resolving titles.
        repair_session_managed_identity(config, &mut fresh);
        ...
        if acknowledge_completion_on_focus(existing, &mut fresh) && !fresh.tmux_pane_id.is_empty() {
            let _ = save_pane_state(
                &config.tmux_state_dir,
                &fresh.tmux_pane_id,
                AgentState::Idle,
...
pub(crate) fn merge_session_refresh_state(
    ...
    hydrate_agent_binding_from_managed_record(config, fresh);
    ...
    fresh.agent_session_id = if fresh.managed {
        // Prefer a fresh poll binding when present. Only fall back to the prior
        // in-memory SID when it is still compatible with the managed agent —
        // otherwise a single poisoned binding (Grok UUID on an OpenCode launch)
        // sticks forever and group_cwd_for_session lists the row under the
        // wrong project.
        let managed_agent = fresh
            .managed_agent
            .as_deref()
            .or(existing.managed_agent.as_deref())
            .unwrap_or("");
        let sid_matches_agent = |sid: &str| {
            crate::agents::agent_session_matches_expected_agent(&config.home, sid, managed_agent)
        };
        let existing_ok = existing.agent_session_id.as_ref().filter(|sid| {
            if !sid_matches_agent(sid) {
                return false;
            }
            // Poll cleared the binding (idle shell + wrong project, cross-agent
            // drop, …). Do not resurrect a SID that no longer matches this pane.
            if fresh.agent_session_id.is_none() {
                return crate::agents::agent_session_matches_pane_cwd(
                    &config.home,
                    &fresh.cwd,
                    sid,
                );
        ...
        let fresh_ok = fresh
            .agent_session_id
            .as_ref()
            .filter(|sid| sid_matches_agent(sid));
        let resolved = fresh_ok
            .cloned()
            .or_else(|| existing_ok.cloned())
            .or_else(|| resolve_agent_session_id(config, existing, fresh, pane_index));
        // resolve_agent_session_id may re-pick existing's poison via ownership
        // checks that do not know about managed-agent type — filter again.
        resolved.filter(|sid| sid_matches_agent(sid))
    } else {
    ...
    if existing.thread_is_complete() {
        ...
        let unconfirmed_completion = !crate::pty::is_sticky_thread_title(&existing.description)
            ...
            || existing.completed_thread.as_deref().is_some_and(|thread| {
                !crate::pty::is_sticky_thread_title(thread)
                    || crate::pty::is_bootstrap_sidebar_thread(thread)
            });
