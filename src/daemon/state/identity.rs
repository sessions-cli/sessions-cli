...
use crate::daemon::state::titles::{
    agent_session_title_is_placeholder, apply_manual_title_identity, ensure_agent_session_title,
    load_auto_persisted_title_identity, reset_unconfirmed_agent_title, session_has_manual_title,
};
...
pub(crate) fn build_previous_ssn_index(
    ...
    for (id, session) in previous {
        if let Some(ref ssn) = session.sessions_session_id {
            by_ssn.entry(ssn.clone()).or_insert_with(|| id.clone());
        }
    }
    by_ssn
}
...
pub(crate) fn find_previous_session<'a>(
    ...
    if let Some(ref ssn) = fresh.sessions_session_id {
        ...
        if let Some(session) = previous
            .values()
            .find(|session| session.sessions_session_id.as_deref() == Some(ssn.as_str()))
        {
            return Some(session);
        }
    }
...
pub(crate) fn copy_stable_refresh_identity(prior: &Session, fresh: &mut Session) {
    ...
    if prior.thread_is_complete() {
        fresh.completed_thread = prior
            .completed_thread
            .clone()
            .or(fresh.completed_thread.clone());
...
/// Repair a session's managed identity so that `managed_agent` and
/// `agent_session_id` stay consistent with the durable managed launch record.
/// This is the recovery path for existing corrupted state where, for example,
/// a Grok hook hijacked an OpenCode-managed window's binding.
pub(crate) fn repair_session_managed_identity(config: &Config, session: &mut Session) {
    let Some(ssn) = session.sessions_session_id.as_deref() else {
        return;
    };
    let Some(record) = crate::session::load_managed_record(&config.home, ssn) else {
        return;
    };
    let record_agent = record.agent.trim().to_ascii_lowercase();
    if !record_agent.is_empty() && record_agent != "console" {
        session.managed_agent = Some(record_agent.clone());
    }

    // Durable record itself may hold a cross-agent SID (written before we
    // validated updates). Scrub it so hydrate cannot re-poison every poll.
    if let Some(ref bound) = record.agent_session_id.clone() {
        if !crate::agents::agent_session_matches_expected_agent(&config.home, bound, &record_agent)
        {
            let _ = crate::session::clear_managed_agent_session_id(
                &config.home,
                ssn,
                Some(bound.as_str()),
            );
    ...
    let mut cleared_sid: Option<String> = None;
    if let Some(ref sid) = session.agent_session_id.clone() {
        if let Some(detected) = crate::agents::detect_agent_id_for_session(&config.home, sid) {
            if detected != record_agent {
                // The managed record was created as "console" but the actual
                // agent is different — repair the record instead of clearing
                // the valid binding.
                if record_agent == "console" && detected != "console" {
                    let mut record = record.clone();
                    record.agent = detected.to_string();
                    let _ = crate::session::save_managed_record(&config.home, &record);
                    session.managed_agent = Some(detected.to_string());
                } else {
                    cleared_sid = Some(sid.clone());
                    session.agent_session_id = None;
                    session.completed_thread = None;
    ...
    // If the visible title was derived from the hijacked agent id, re-resolve
    // it under the managed agent so the sidebar does not keep a stale prefix.
    if let Some(cleared_sid) = cleared_sid {
        // Hijacked SID's project must not keep the row under the wrong group.
        session.cwd_label = crate::pty::format_tilde_path(&session.cwd, &config.home);
        if !session.title_manual {
            if let Some(detected) =
                crate::agents::detect_agent_id_for_session(&config.home, &cleared_sid)
            {
                let title_app = crate::pty::parse_app(&session.title);
                if title_app
                    .as_deref()
                    .is_some_and(|app| app.eq_ignore_ascii_case(detected))
                {
                    let workspaces =
                        crate::session::WorkspaceCatalog::load(&config.workspaces_path);
                    let workspace =
                        workspaces.workspace_ref_for_window(session.tab_index, &session.cwd);
                    let (title, description, project) = crate::pty::resolve_session_names(
                        &config.home,
                        &session.cwd,
                        Some(&record_agent),
                        None,
...
/// Restore agent bindings from the durable managed launch record before merge
/// treats a managed Grok/Codex window as a bare console shell.
pub(crate) fn hydrate_agent_binding_from_managed_record(config: &Config, session: &mut Session) {
    let Some(ssn) = session.sessions_session_id.as_deref() else {
        return;
    };
    let Some(record) = crate::session::load_managed_record(&config.home, ssn) else {
    ...
    let record_agent = record.agent.trim();
    if session.agent_session_id.is_none() {
        if let Some(ref sid) = record.agent_session_id {
            // Never rehydrate a cross-agent poison pill from disk.
            if crate::agents::agent_session_matches_expected_agent(&config.home, sid, record_agent)
            {
    ...
    if !record_agent.is_empty()
        && record_agent != "console"
        && crate::pty::is_agent_app(record_agent)
    {
        session.managed_agent = Some(record_agent.to_ascii_lowercase());
        return;
    }
    if session.managed_agent.is_some() {
        return;
    }
    if let Some(sid) = session.agent_session_id.as_deref() {
        if let Some(detected) = crate::agents::detect_agent_id_for_session(&config.home, sid) {
            session.managed_agent = Some(detected.to_string());
        }
    }
...
pub(crate) fn finalize_session_identity(config: &Config, existing: &Session, fresh: &mut Session) {
    ...
    let needs_confirmed_title = fresh.agent_session_id.is_some()
        && (crate::pty::is_machine_derived_thread(&fresh.description)
            || crate::pty::is_machine_derived_thread(&existing.description)
            || crate::pty::is_machine_derived_thread(&crate::pty::parse_description(&fresh.title))
