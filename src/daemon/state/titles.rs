...
pub(crate) fn ensure_agent_session_title(config: &Config, existing: &Session, session: &mut Session) {
...
pub(crate) fn apply_manual_title_identity(config: &Config, existing: &Session, fresh: &mut Session) -> bool {
...
pub(crate) fn restore_manual_title_from_tab(config: &Config, tab_index: u32, session: &mut Session) -> bool {
...
pub(crate) fn clear_manual_title_files_for_tab(config: &Config, tab_index: u32) -> std::io::Result<()> {
...
pub(crate) fn restore_title_from_tab_index(config: &Config, tab_index: u32, session: &mut Session) -> bool {
    let Ok(data) = std::fs::read_to_string(config.session_title_path_for_tab(tab_index)) else {
        return false;
    };
    let title = data.trim();
...
pub(crate) fn restore_title_from_disk(config: &Config, agent_session_id: &str, session: &mut Session) -> bool {
