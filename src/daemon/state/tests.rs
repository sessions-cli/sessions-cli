...
/// OpenCode launched at ~ must not stay grouped under acme because a
/// Grok UUID was written into the managed launch record.
#[test]
fn repair_clears_cross_agent_managed_binding_and_resets_group() {
    let dir = TempDir::new().unwrap();
    let mut config = Config::default();
    config.home = dir.path().to_path_buf();
    let home_cwd = "/home/testuser";
    let acme_cwd = "/home/testuser/projects/acme";
    let grok_sid = "019f1ab1-858d-7120-b097-3309bfd5b6c5";
    let summary_dir = crate::agents::grok::session_dir(&config.home, acme_cwd, grok_sid);
    std::fs::create_dir_all(&summary_dir).unwrap();
    std::fs::write(
        summary_dir.join("summary.json"),
        format!(r#"{{"info":{{"cwd":"{acme_cwd}"}}}}"#),
    )
    .unwrap();

    let record = crate::session::ManagedLaunchRecord {
        sessions_session_id: "ssn_opencode_home".into(),
        launch_id: "lch_opencode_home".into(),
        agent: "opencode".into(),
        tmux_session: "agents".into(),
        window_index: 5,
        pane_id: Some("%34".into()),
        initial_cwd: home_cwd.into(),
        created_at: Utc::now().to_rfc3339(),
        agent_session_id: Some(grok_sid.into()),
        pool: false,
    };
    crate::session::save_managed_record(&config.home, &record).unwrap();
    ...
    let mut session = Session {
        managed: true,
        sessions_session_id: Some("ssn_opencode_home".into()),
        managed_agent: Some("opencode".into()),
        agent_session_id: Some(grok_sid.into()),
        cwd: home_cwd.into(),
        cwd_label: "~/projects/acme".into(),
        title: "opencode · any luck".into(),
        description: "any luck".into(),
        project: "opencode".into(),
        tmux_pane_id: "%34".into(),
        tab_index: 5,
        ..sample_entry("any luck")
    };

    repair_session_managed_identity(&config, &mut session);

    assert!(
        session.agent_session_id.is_none(),
        "cross-agent Grok SID must be cleared from the OpenCode row"
    );
    assert!(
        !session.cwd_label.contains("acme"),
        "group must not stay under the hijacked Grok project, got {}",
        session.cwd_label
    );
    assert_eq!(
        session.cwd_label,
        crate::pty::format_tilde_path(home_cwd, &config.home),
        "group must fall back to pane cwd after clearing the acme SID"
    );
    let scrubbed = crate::session::load_managed_record(&config.home, "ssn_opencode_home").unwrap();
    assert!(
        scrubbed.agent_session_id.is_none(),
        "durable managed record must not re-poison on the next hydrate"
    );
}

#[test]
fn merge_drops_cross_agent_existing_binding_on_managed_opencode() {
    let dir = TempDir::new().unwrap();
    let mut config = Config::default();
    config.home = dir.path().to_path_buf();
    let home_cwd = "/home/testuser";
    let acme_cwd = "/home/testuser/projects/acme";
    let grok_sid = "019f1ab1-858d-7120-b097-3309bfd5b6c6";
    let summary_dir = crate::agents::grok::session_dir(&config.home, acme_cwd, grok_sid);
    std::fs::create_dir_all(&summary_dir).unwrap();
    std::fs::write(
        summary_dir.join("summary.json"),
    ...
    let record = crate::session::ManagedLaunchRecord {
        sessions_session_id: "ssn_merge_oc".into(),
        launch_id: "lch_merge_oc".into(),
        agent: "opencode".into(),
        tmux_session: "agents".into(),
        window_index: 5,
        pane_id: Some("%34".into()),
    ...
    let existing = Session {
        managed: true,
        sessions_session_id: Some("ssn_merge_oc".into()),
        managed_agent: Some("opencode".into()),
        agent_session_id: Some(grok_sid.into()),
        cwd: home_cwd.into(),
        cwd_label: "~/projects/acme".into(),
        tmux_pane_id: "%34".into(),
        tab_index: 5,
        ..sample_entry("poisoned")
    };
    // Poll already dropped the cross-agent SID.
    let mut fresh = Session {
        managed: true,
        sessions_session_id: Some("ssn_merge_oc".into()),
        managed_agent: Some("opencode".into()),
    ...
    merge_session_refresh_state(
        ...
        &existing,
        &mut fresh,
        &HashMap::from([("%34".to_string(), 5)]),
        &WorkspaceCatalog::default(),
    );

    assert!(
        fresh.agent_session_id.is_none(),
        "merge must not resurrect the Grok SID onto the OpenCode managed row"
    );
    assert!(
        !fresh.cwd_label.contains("acme"),
        "group must not stay under the hijacked project, got {}",
        fresh.cwd_label
    );
}
...
fn resolve_renamed_title_accepts_full_display_title() {
    ...
    let (title, description, project) = resolve_renamed_title(&session, "codex · refreshed task");
...
fn session_start_clears_prompt_submitted() {
    // Brand-new session (no messaged_at): session_start applies and clears the flag.
    let mut entry = sample_entry("ship api");
...
#[test]
fn session_start_noop_on_existing_messaged_session() {
    // Prompt-less session_start on an existing session is a restore artifact — leave
    // ordering timestamps and prompt_submitted alone.
    let mut entry = sample_entry("ship api");
    ...
    assert!(entry.prompt_submitted);
    assert_eq!(entry.messaged_at, messaged_at);
}
...
fn session_start_sets_messaged_at_for_group_order() {
    // Only brand-new sessions (no prior messaged_at) get stamped for group order.
    let mut entry = sample_entry("ship api");
    entry.messaged_at = None;
    assert_eq!(
        apply_thread_hook(&mut entry, "session_start", AgentState::Idle),
...
fn refresh_merge_relabels_workspace_when_foreground_tool_changes() {
    ...
    existing.title = "acme · frontend dev".into();
    existing.project = "acme".into();
    existing.cwd = "/home/testuser/projects/acme/acme-frontend".into();
    let mut fresh = sample_entry("htop");
    fresh.title = "htop".into();
    fresh.project = "acme".into();
...
fn refresh_merge_restores_grok_summary_for_placeholder_managed_session() {
    ...
    std::fs::write(
        ...
        r#"{
  "info": {"id": "00000000-0000-4000-8000-000000000001", "cwd": "/home/testuser/projects/demo-cli"},
  "session_summary": "Control N New Section Failure in Active Project Background Process"
    ...
    std::fs::write(
            session_dir.join("events.jsonl"),
            r#"{"ts":"2026-06-12T10:49:59.194Z","type":"turn_started","session_id":"00000000-0000-4000-8000-000000000001"}"#,
        )
        .unwrap();
    std::fs::create_dir_all(config.grok_state_dir()).unwrap();
    std::fs::write(
...
#[test]
fn refresh_merge_hydrates_managed_grok_binding_from_record() {
    let dir = TempDir::new().unwrap();
    let mut config = Config::default();
    config.home = dir.path().to_path_buf();

    let sid = "00000000-0000-4000-8000-000000000002";
    let ssn = "ssn_managed_grok_console";
    let cwd = "/home/testuser/projects/gadget-lab";
    let summary_dir = grok_session_dir(&config.home, cwd, sid);
    std::fs::create_dir_all(&summary_dir).unwrap();
    std::fs::write(
        summary_dir.join("summary.json"),
        r#"{
  "info": {"id": "00000000-0000-4000-8000-000000000002", "cwd": "/home/testuser/projects/gadget-lab"},
  "generated_title": "ADB Root Automation for Time-Based Essential App Restrictions Android"
}"#,
    )
    .unwrap();
    write_grok_turn_started(&config, cwd, sid);
    ...
    let existing = Session {
        agent_session_id: Some(sid.into()),
        title: "grok · ADB Root Automation for Time-Based Essential App Restrictions Android"
            .into(),
        description: "ADB Root Automation for Time-Based Essential App Restrictions Android".into(),
        project: "grok".into(),
        managed: true,
    ...
    assert_eq!(fresh.agent_session_id.as_deref(), Some(sid));
    assert_eq!(
        fresh.description,
        "ADB Root Automation for Time-Based Essential App Restrictions Android"
    );
    assert_eq!(
        fresh.title,
        "grok · ADB Root Automation for Time-Based Essential App Restrictions Android"
    );
    assert_eq!(fresh.state, AgentState::Working);
}
...
fn compute_refresh_merges_by_sessions_session_id_preserves_order() {
    ...
    let previous = HashMap::from([(prior_a.id.clone(), prior_a), (prior_b.id.clone(), prior_b)]);
...
fn compute_refresh_rejects_direct_index_match_when_sessions_session_id_changes() {
    ...
    assert_eq!(
        out.merged
            .iter()
            .filter(|session| { session.sessions_session_id.as_deref() == Some("ssn_moved") })
            .count(),
        1
    );
}
...
fn compute_refresh_hydrates_manifest_messaged_at_without_stamp_new_order() {
    ...
    assert_eq!(merged.agent_session_id.as_deref(), Some("agent-manifest"));
}
...
fn compute_refresh_stamps_unmanaged_session_without_manifest_entry() {
    ...
    assert!(out.merged[0].messaged_at.unwrap() > Utc::now() - chrono::Duration::seconds(2));
}
...
fn compute_refresh_filters_closed_but_reports_all_polled_identities() {
    ...
    win1.sessions_session_id = Some("ssn_1".into());
    ...
    win2.sessions_session_id = Some("ssn_2".into());

    // win2 is marked closed by pane only (SSN already absent from live poll
    // identity would heal). Pane marker must still suppress during in-flight close.
    let mut closed = std::collections::HashSet::new();
    closed.insert(ClosedSessionMarker {
        sessions_session_id: None,
...
fn hook_targets_session_rejects_subagent_session_start_for_parent_pane() {
    ...
    std::fs::write(
        ...
        format!(r#"{{"parent_session_id":"{parent}","child_session_id":"{subagent}"}}"#),
    )
    .unwrap();
...
#[test]
fn hook_targets_session_rejects_grok_session_start_over_opencode() {
    let entry = Session {
        agent_session_id: Some("sid-opencode".into()),
        managed_agent: Some("opencode".into()),
        tmux_pane_id: "%7".into(),
        title: "opencode · refactor auth".into(),
        ..sample_entry("refactor auth")
    };
    let msg = NotifyMessage {
        t: "notify".into(),
        agent: Some("grok".into()),
        session_id: Some("sid-grok".into()),
        tmux_pane_id: Some("%7".into()),
        tmux_session: Some("agents".into()),
        event: "session_start".into(),
        ts: 0,
        payload: serde_json::json!({}),
...
#[test]
fn hook_targets_session_rejects_grok_session_start_over_inferred_codex_title() {
    let entry = Session {
        agent_session_id: Some("sid-codex".into()),
        managed_agent: None,
        tmux_pane_id: "%8".into(),
        title: "codex · fix sidebar".into(),
        ..sample_entry("fix sidebar")
    };
    let msg = NotifyMessage {
        t: "notify".into(),
...
#[test]
fn hook_targets_session_allows_session_start_to_bind_console_pane() {
    let entry = Session {
        agent_session_id: None,
        managed_agent: None,
        tmux_pane_id: "%10".into(),
        title: "console".into(),
        description: "console".into(),
        ..sample_entry("console")
    };
...
#[test]
fn hook_targets_session_allows_grok_session_start_to_replace_old_grok() {
    let entry = Session {
        agent_session_id: Some("sid-old".into()),
        managed_agent: Some("grok".into()),
        tmux_pane_id: "%3".into(),
        title: "grok · old thread".into(),
        ..sample_entry("old thread")
    };
    let msg = NotifyMessage {
        t: "notify".into(),
...
fn refresh_merge_groups_cross_project_grok_under_agent_cwd() {
    ...
    assert_eq!(fresh.cwd, acme_cwd);
}

#[test]
fn refresh_merge_clears_cross_project_grok_on_idle_acme_shell() {
    let dir = TempDir::new().unwrap();
    let mut config = Config::default();
    config.home = dir.path().to_path_buf();
    ...
    assert_eq!(fresh.cwd_label, format_tilde_path(acme_cwd, &config.home));
...
#[test]
fn resolve_window_index_prefers_managed_record_over_stale_agent_env() {
    use crate::model::NotifyMessage;
    ...
    std::fs::create_dir_all(config.grok_state_dir()).unwrap();
    std::fs::write(
        config.session_env_path("ses_stale"),
        "TMUX_PANE=%26\nSESSIONS_WINDOW_INDEX=26\nTMUX_SESSION=agents\n",
    )
    .unwrap();
    let record = ManagedLaunchRecord {
        sessions_session_id: "ssn_live".into(),
        launch_id: "lch_test".into(),
        agent: "opencode".into(),
        tmux_session: "agents".into(),
        window_index: 29,
        pane_id: Some("%31".into()),
        initial_cwd: "/tmp/project-a".into(),
        created_at: Utc::now().to_rfc3339(),
        agent_session_id: None,
        pool: false,
    };
    ...
    let msg = NotifyMessage {
        ...
        kitty_pid: None,
        kitty_listen_on: None,
        sessions_session_id: Some("ssn_live".into()),
        ..Default::default()
    };
    assert_eq!(resolve_window_index(&msg, &config), Some(29));
}
...
#[test]
fn closed_session_marker_matches_stable_ssn_not_agent_sid_alone() {
    let marker = ClosedSessionMarker {
        sessions_session_id: Some("ssn_a".into()),
        agent_session_id: Some("sid-a".into()),
        tmux_pane_id: "%1".into(),
    };
    let same_ssn = Session {
        sessions_session_id: Some("ssn_a".into()),
        agent_session_id: Some("sid-a".into()),
        tmux_pane_id: "%9".into(),
        ..sample_entry("task")
    };
    let other_ssn_same_agent = Session {
        sessions_session_id: Some("ssn_b".into()),
        agent_session_id: Some("sid-a".into()),
        tmux_pane_id: "%9".into(),
        ..sample_entry("other")
    };
    assert!(marker.matches(&same_ssn));
    assert!(
        !marker.matches(&other_ssn_same_agent),
        "agent_session_id alone must not hide a different live managed session"
    );
    // Late hooks after close still match by agent sid / pane.
...
#[test]
fn sessions_session_ids_to_tombstone_skips_ssn_moved_to_new_window_id() {
    let mut previous_row = sample_entry("moved");
    previous_row.id = "tmux:win:5".into();
    previous_row.tab_index = 5;
    previous_row.sessions_session_id = Some("ssn_moved".into());
    let previous = HashMap::from([(previous_row.id.clone(), previous_row)]);

    let mut live_row = sample_entry("moved");
    live_row.id = "tmux:win:17".into();
    live_row.tab_index = 17;
    live_row.sessions_session_id = Some("ssn_moved".into());
    let live = HashMap::from([(live_row.id.clone(), live_row)]);

    let vanished = sessions_session_ids_to_tombstone(&previous, &live);
    assert!(
        vanished.is_empty(),
        "stable SSN reattached under a new tmux:win:N must not be tombstoned: {vanished:?}"
    );
}

#[test]
fn sessions_session_ids_to_tombstone_includes_ssn_truly_gone() {
    let mut previous_row = sample_entry("gone");
    previous_row.id = "tmux:win:5".into();
    previous_row.sessions_session_id = Some("ssn_gone".into());
    let previous = HashMap::from([(previous_row.id.clone(), previous_row)]);

    let mut other = sample_entry("other");
    other.id = "tmux:win:1".into();
    other.sessions_session_id = Some("ssn_other".into());
    let live = HashMap::from([(other.id.clone(), other)]);

    let vanished = sessions_session_ids_to_tombstone(&previous, &live);
    assert_eq!(vanished, vec!["ssn_gone".to_string()]);
}

#[test]
fn compute_refresh_heals_closed_marker_when_ssn_still_live() {
    let config = Config::default();
    let mut win = sample_entry("still live");
    win.id = "tmux:win:1".into();
    win.tab_index = 1;
    win.tmux_pane_id = "%1".into();
    win.managed = true;
    win.sessions_session_id = Some("ssn_live".into());
    win.agent_session_id = Some("sid-live".into());

    let mut closed = std::collections::HashSet::new();
    closed.insert(ClosedSessionMarker {
        sessions_session_id: Some("ssn_live".into()),
        agent_session_id: Some("sid-live".into()),
    ...
    assert_eq!(
        out.merged.len(),
        1,
        "live closed SSN must reappear in merge"
    );
    assert_eq!(
        out.merged[0].sessions_session_id.as_deref(),
        Some("ssn_live")
    );
    assert_eq!(out.healed_closed_ssns, vec!["ssn_live".to_string()]);
}

#[test]
fn compute_refresh_heals_in_flight_close_when_window_still_live() {
    let config = Config::default();
    let mut open = sample_entry("open");
    open.id = "tmux:win:1".into();
    open.tab_index = 1;
    open.tmux_pane_id = "%1".into();
    open.sessions_session_id = Some("ssn_open".into());
    open.agent_session_id = Some("sid-open".into());

    // Close marked the SSN but the window is still in this poll (kill lag / failed kill).
    let mut closing = sample_entry("closing");
    closing.id = "tmux:win:2".into();
    closing.tab_index = 2;
    closing.tmux_pane_id = "%2".into();
    closing.sessions_session_id = Some("ssn_closing".into());
    closing.agent_session_id = Some("sid-closing".into());

    let mut closed = std::collections::HashSet::new();
    closed.insert(ClosedSessionMarker::from_session(&closing));

    let out = compute_refresh(
        &config,
        vec![open, closing.clone()],
        &HashMap::new(),
        &closed,
        &WorkspaceCatalog::default(),
    );

    // Live SSN wins over a stale tombstone so rows do not permanently vanish.
    assert_eq!(out.merged.len(), 2);
    assert!(out.healed_closed_ssns.contains(&"ssn_closing".to_string()));
}

#[test]
fn compute_refresh_does_not_filter_by_agent_sid_from_unrelated_closed_entry() {
    let config = Config::default();
    let mut live = sample_entry("live");
    live.id = "tmux:win:1".into();
    live.tab_index = 1;
    live.tmux_pane_id = "%1".into();
    live.managed = true;
    live.sessions_session_id = Some("ssn_live".into());
    live.agent_session_id = Some("sid-shared".into());

    // Stale closed entry reused the same agent sid under a different SSN.
    let mut closed = std::collections::HashSet::new();
    closed.insert(ClosedSessionMarker {
        sessions_session_id: Some("ssn_old_closed".into()),
        agent_session_id: Some("sid-shared".into()),
        tmux_pane_id: String::new(),
    });
    ...
    assert!(out.healed_closed_ssns.is_empty());
}
...
async fn set_booting_resets_after_restore_complete() {
    ...
    assert!(
        state.is_booting().await,
        "daemon starts booting until reconcile"
    );
