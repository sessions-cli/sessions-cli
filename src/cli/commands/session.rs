...
use crate::session::{self, CloseOutcome, CloseTarget};
use crate::telemetry;
use crate::version;
use anyhow::Result;
...
pub fn run_focus(socket: Option<PathBuf>, index: u32) -> Result<()> {
    telemetry::record_feature(
        telemetry::FeatureId::ListFocusNumber,
...
/// Default keyboard step for `sessions resize-sidebar wider|narrower`.
const RESIZE_SIDEBAR_DEFAULT_STEP: u16 = 4;

/// Adjust the sessions-ui sidebar pane width without requiring bar focus.
///
/// `direction`: `wider` / `narrower` (aliases: `+`/`grow`, `-`/`shrink`).
/// Optional `step` overrides the default 4-column step.
pub fn run_resize_sidebar(direction: &str, step: Option<u16>) -> Result<()> {
    let config = Config::default();
    let step = i16::try_from(step.unwrap_or(RESIZE_SIDEBAR_DEFAULT_STEP)).unwrap_or(4);
    let delta = match direction.trim().to_ascii_lowercase().as_str() {
        "wider" | "wide" | "grow" | "+" | "right" | "]" => step,
        "narrower" | "narrow" | "shrink" | "-" | "left" | "[" => -step,
        other => anyhow::bail!("unknown resize direction {other:?}; use wider or narrower"),
    };
    daemon::tmux::resize_ui_sidebar_by(&config.tmux_ui_session, delta)?;
    Ok(())
}

pub fn run_status(socket: Option<PathBuf>, verbose: bool, json: bool) -> Result<()> {
    telemetry::record_feature(
        telemetry::FeatureId::CliStatus,
        telemetry::feature::Source::Cli,
    );
    let config = config_with_socket(socket);
...
pub fn run_panel_automations() -> Result<()> {
    let config = Config::default();
    daemon::tmux::open_workspace_automations(&config.tmux_ui_session, &config.tmux_session)?;
    Ok(())
}

pub fn run_panel_mcps() -> Result<()> {
    let config = Config::default();
    daemon::tmux::open_workspace_mcps(&config.tmux_ui_session, &config.tmux_session)?;
    Ok(())
}

pub fn run_panel_open_mcps() -> Result<()> {
    let config = Config::default();
    daemon::tmux::open_workspace_mcps(&config.tmux_ui_session, &config.tmux_session)?;
    Ok(())
}

pub fn run_panel_skills() -> Result<()> {
    let config = Config::default();
    daemon::tmux::open_workspace_skills(&config.tmux_ui_session, &config.tmux_session)?;
    Ok(())
}

pub fn run_panel_open_skills() -> Result<()> {
    let config = Config::default();
    daemon::tmux::open_workspace_skills(&config.tmux_ui_session, &config.tmux_session)?;
    Ok(())
}
...
pub(crate) fn flush_and_sync_manifest_before_down(config: &Config) -> Result<()> {
    let sessions = daemon_sessions_for_down_sync(config);
    session::sync_manifest_from_daemon_snapshot(config, &sessions)?;

    // Live daemon list is the truth of "what was in sessions before down".
    // Tombstone open manifest rows that were already gone so they do not restore
    // as phantom PWD groups, and pin the restore allowlist + sidebar group order.
    let live_ssns: std::collections::HashSet<String> = sessions
        .iter()
        .filter_map(|session| session.sessions_session_id.clone())
        .collect();
    let _ = session::tombstone_open_entries_absent_from(config, &live_ssns);
    let _ = session::live_snapshot::save_from_sessions(config, &sessions);

    let mut group_order = session::group_order::load(config).groups;
    let _ = session::group_order::reconcile_and_save(config, &mut group_order, &sessions);
    Ok(())
}
...
pub fn run_up(no_attach: bool) -> Result<()> {
    let config = Config::default();
    let _ = telemetry::ensure_sessions_config(&config.home);
    telemetry::record_feature(telemetry::FeatureId::CliUp, telemetry::feature::Source::Cli);
    let _ = telemetry::heartbeat::maybe_heartbeat(false);
    ensure_agent_hooks(&config);
    // skillshare / Obot are optional — set up from Skills / MCP panels when needed.
    session::orchestrate_up_with_options(&config, !no_attach)
}
...
mod tests {
    ...
    fn confirm_close_tombstones_manifest() {
        ...
        assert!(
            entry.closed,
            "confirm-close path must tombstone manifest entry"
        );
    ...
    fn run_down_ordering_sync_before_kill() {
        ...
        // Orphan open entry that is not live should be tombstoned on down.
        crate::session::append_entry(
            &config,
            crate::session::ManifestEntry {
                sessions_session_id: "ssn_orphan".into(),
                source: crate::session::ManifestSource::Cli,
                workspace_index: None,
                cwd: "/tmp/orphan".into(),
                cwd_label: "/tmp/orphan".into(),
                agent: "grok".into(),
                launch_command: "grok".into(),
                agent_session_id: None,
                title: None,
        ...
        let orphan = crate::session::load_manifest(&config)
            .unwrap()
            .entries
            .into_iter()
            .find(|entry| entry.sessions_session_id == "ssn_orphan")
            .expect("orphan entry");
        assert!(
            orphan.closed,
            "down flush must tombstone open entries that were not live"
        );

        let allow = crate::session::live_snapshot::load(&config).expect("live snapshot");
        assert!(allow.contains("ssn_order"));
        assert!(!allow.contains("ssn_orphan"));

        let group_order = crate::session::group_order::load(&config);
        assert!(
            group_order.groups.iter().any(|label| label == "/tmp/order"),
            "down flush must persist live PWD order; got {:?}",
            group_order.groups
        );
