...
pub use launcher::{
    agent_accepts_cli_prompt, agent_session_exists, build_launch_command,
    build_launch_command_with_prompt,
    build_quick_launch_command, build_resume_command, contains_prompt, default_model_id, deliver_prompt_via_tmux,
    effective_restore_command, is_workspace_script, looks_like_shell_command, model_index,
    normalized_launch_command, parse_model_from_launch_command, AgentEntry, AGENTS,
};
pub use opencode::{
    assign_session_for_cwd, is_opencode_session_id, opencode_session_index, OpenCode,
};
pub use registry::{
    hook_provider_by_id, launch_provider_by_id, provider_by_id, providers_by_detection_priority,
    ProviderDescriptor, AGENT_IDS, PROVIDERS,
};
pub use traits::hooks::{AgentHookReport, HookProvider};
pub use traits::launch::{LaunchProvider, ModelOption};
...
{{"type":"event_msg","payload":{{"type":"user_message","message":"fix codex sidebar titles"}}}}"#
    ...
    fn group_cwd_for_session_uses_agent_project_not_pane() {
        ...
        assert_eq!(
            group_cwd_for_session(home, superflip_cwd, Some(sid)),
            sessions_cwd
        );
        assert_eq!(
            group_cwd_for_session(home, superflip_cwd, None),
            superflip_cwd
        );
    }
    ...
    fn agent_session_matches_pane_cwd_uses_summary_project() {
        ...
        assert!(!agent_session_matches_pane_cwd(home, superflip_cwd, sid));
        assert_eq!(
            grok::load_session_summary(home, superflip_cwd, sid)
