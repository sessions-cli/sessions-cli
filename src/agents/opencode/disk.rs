...
mod tests {
    ...
    fn turn_boundary_reads_user_and_stop_messages() {
        ...
        let boundary = OpenCode
            .turn_boundary(home, env!("CARGO_MANIFEST_DIR"), "ses_test")
            .unwrap();
        assert_eq!(
            boundary.last_started.map(|t| t.timestamp_millis()),
            Some(100)
        );
        assert_eq!(
            boundary.last_completed.map(|t| t.timestamp_millis()),
            Some(900)
        );
