...
pub fn status(home: &Path) -> OpenCodeHookStatus {
    ...
    let plugin_enabled =
        config_path(home).is_file() && plugin_enabled_in_config(&config_path(home));
    let plugin_current = plugin_installed && plugin_export_is_function(&plugin);

    let health = if !plugin_installed {
        OpenCodeHookHealth::NotFound
    } else if !plugin_current {
        // Old object export `{ id, server }` fails with "Plugin export is not a function".
    ...
    OpenCodeHookStatus {
        health,
        plugin_installed,
        plugin_enabled: plugin_enabled && plugin_current,
    }
}

/// OpenCode requires a default-exported plugin *function*. Object module shapes break load.
fn plugin_export_is_function(plugin: &Path) -> bool {
    let Ok(raw) = std::fs::read_to_string(plugin) else {
        return false;
    };
    // Current install: `export default SessionsPlugin` (function).
    // Broken historical shape: `export default { id: "sessions", server: ... }`.
    let object_export = raw.contains("export default {") || raw.contains("server: SessionsPlugin");
    let function_export = raw.contains("export default SessionsPlugin");
    function_export && !object_export
}
...
pub fn detail_label(status: &OpenCodeHookStatus) -> String {
    match status.health {
        ...
        OpenCodeHookHealth::Configured => "configured (question popup bypass)".into(),
        OpenCodeHookHealth::OutOfDate => {
            "plugin out of date — run: sessions hooks setup opencode".into()
        }
    }
}
...
mod tests {
    ...
    fn setup_installs_opencode_plugin() {
        ...
        let plugin = fs::read_to_string(plugin_dir(home).join("sessions.ts")).unwrap();
        assert!(
            plugin.contains("export default SessionsPlugin"),
            "plugin must default-export a function"
        );
        assert!(
            !plugin.contains("server: SessionsPlugin"),
            "object module export breaks OpenCode loaders"
        );
        assert!(
            plugin.contains("question.asked"),
            "plugin must intercept OpenCode questions"
        );
    }

    #[test]
    fn detects_broken_object_plugin_export() {
        let dir = tempdir().unwrap();
        let home = dir.path();
        fs::create_dir_all(plugin_dir(home)).unwrap();
        fs::write(
            plugin_dir(home).join("sessions.ts"),
            r#"export default { id: "sessions", server: SessionsPlugin };"#,
        )
        .unwrap();
        fs::write(plugin_dir(home).join("package.json"), PACKAGE_JSON).unwrap();
        fs::write(config_path(home), r#"{"plugin":["sessions"]}"#).unwrap();
        let current = status(home);
        assert_eq!(current.health, OpenCodeHookHealth::OutOfDate);
    }
}
